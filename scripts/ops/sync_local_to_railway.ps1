...
Write-Host "`n[3/3] Running scripts/ops/railway_bootstrap.py ..." -ForegroundColor Green
if ($RemoteUrl) {
    $env:DATABASE_URL = $RemoteUrl
    python scripts/ops/railway_bootstrap.py
} else {
    railway run python scripts/ops/railway_bootstrap.py
}
if ($LASTEXITCODE -ne 0) {
