"""channel 전역 주문 생성 ENABLE CLI (operation CHANNEL_CREATE_ENABLE).

    python tools/ops/channel_create_enable.py \
        --rollout-artifact <readiness json> --expected-version <flag version> \
        --approval-token-file <under-control-root>.json --apply

전역 create flag 를 DISABLED→ENABLED 로 전이하고 PAUSED_ACCEPTED receipt 를 ACCEPTED 로
되살린다(resume). owner-only approval 토큰을 소비하며 기본 dry-run.
"""
...
OPS_APPROVAL_OPERATION_ID = "CHANNEL_CREATE_ENABLE"


def main(argv: "list[str] | None" = None) -> int:
    """CLI 진입점 — 전역 생성 flag 를 ENABLE 하고 paused receipt 를 resume 한다."""
    parser = argparse.ArgumentParser(description="Enable channel order creation (global flag).")
    ...
    scope = create_flag.build_scope(OPS_APPROVAL_OPERATION_ID, "enable", art_sha, args.expected_version)

    def _build(session, approver_id):
        return create_flag.enable(
