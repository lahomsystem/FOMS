"""통화 결과 기록(B1) mutation handler for the canonical orders blueprint.

`POST /api/orders/<id>/call-log` — sd['calls']에 통화 결과를 append하고,
measurement_date가 오면 schedule.measurement.date를 갱신한다. OrderEvent(CALL_LOGGED)
및 실측일 변경 시 MEASUREMENT_DATE_CHANGED를 기록한다.
"""
...
def _parse_payload(
    ...
    error가 not None이면 400. measurement_date는 ISO(YYYY-MM-DD)만 허용.
    """
...
def log_call_response(order_id: int) -> Any:
    """통화 결과를 기록하고 필요 시 실측일을 갱신한다.

    Body: {result, memo?, measurement_date?(ISO)}.
    반환: {success, data:{call, calls_count}} 또는 오류 {success:false, error}.
    """
    db = get_db()
    ...
    try:
        ...
        last_call = _append_call(sd, result, memo, user_id, _resolve_by_name(db, user_id))
        ...
        return jsonify({"success": True, "data": {"call": last_call, "calls_count": len(sd["calls"])}})
...
__all__ = ["CALL_RESULTS", "log_call_response"]
