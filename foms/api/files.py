"""
Canonical files API implementation (Wave 3).

Canonical target: this module (`foms.api.files`).
Retirement: `apps.api.files` remains a thin import shim until all consumers import
from `foms.api.files` directly (target: Wave 8 legacy bridge retirement per modular
monolith spec).
Removal condition: grep shows no `from apps.api.files` / `apps.api import files` in
product code; then delete shim and update `blueprints.py` import in a dedicated wave.
"""
