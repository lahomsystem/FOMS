...
의 ``include_failed`` / ``pending_retry_before`` 참조.
"""
...
def build_missing_geocode_query(
    ...
    *,
    ...
    Args:
        ...
        include_failed: True 면 ``geocode_status='failed'`` 건도 대상에 넣는다.
            기본 False — 주소 자체가 틀린 건이 대부분이라 반복 호출은 카카오 쿼터만
            태운다(백필 CLI ``--include-failed`` 와 같은 이유).
