    ...
    def __init__(self):
        ...
        if self.storage_type in ["r2", "s3"]:
            ...
            try:
                ...
                if BOTO3_AVAILABLE and BotocoreConfig:
                    print(
                        f"[INFO] [OK] {self.storage_type.upper()} 스토리지가 활성화되었습니다. "
                        f"(connect={_connect}s, read={_read}s)"
                    )
                else:
        ...
        try:
            if self.storage_type in ["r2", "s3"]:
                ...
                return {
                    ...
                    else self.get_download_url(thumbnail_key),
                }
            else:
        ...
        try:
            if file_type == "image" and PILLOW_AVAILABLE:
                ...
                if self.storage_type in ["r2", "s3"]:
                    ...
                    return (
                        self._get_public_url(thumbnail_key)
                        if self._is_public_bucket()
                        else self.get_download_url(thumbnail_key)
                    )
                else:
