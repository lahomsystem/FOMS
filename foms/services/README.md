...
3. `tests/test_foms_namespace_imports.py` — 루트 shim ↔ flat canonical import 동치 스모크 (파일럿 순서가 아님)
...
`business_calendar`는 controlling spec §1.2.16에 따라 **Wave 6 mainline 코드 파일럿 대상이 아니다** (explicit exception + 승인 게이트). 향후 목표 경로만 문서로 고정: `foms/services/common/business_calendar.py`.
...
| Flat compat | `foms/services/realtime_notifications.py` — 패키지로 re-export |
| Root compat | `services/realtime_notifications.py` — 패키지로 re-export |
...
| **`common`** | (예외) `business_calendar.py` — 루트 `services/`에 live 구현 | 승인 전까지 구조 이동 금지 |
...
- **예외:** `services/business_calendar.py`는 여전히 **루트에 live 구현**이 남아 있으며, live import debt의 중심이다 (`W6-B0` import debt 표 참조).
...
|------|-----------|-------------------------------|
| Pilot 완료 | `notifications`, `files` (helper) | canonical package 경로 확정; flat/root = shim-only |
| 이미 패키지 선례 | `jobs`, `erp_policy` public 패턴 | completed — 본 wave 이동 대상 아님 |
| Explicit exception | `business_calendar` / `/calendar` | not started — spec §1.2.16 승인 게이트 |
