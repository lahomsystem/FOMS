...
#: ``사이즈``\ ·``규격``\ ·``폭`` 은 **W 칸에서는 여전히 뺀다** — 그 값(``150``\ ·``180cm``)은
#: 고객이 고른 모듈 폭이지 ERP 총폭이 아니다(총폭 = 모듈×수량 + 길이추가, 실사례
#: 30cm×12 + 1cm×12 = 3,720mm). ``150`` 을 W 칸에 넣으면 2,220mm 를 잃는다 — W 칸을 채울
#: 자격은 화면이 만드는 총폭 칩뿐이다.
#:
#: 대신 2026-09-08 담당자 지시로 **사이즈 키는 제품명 칸으로** 간다. 사이즈만 있고 ``제품``
#: 키가 없는 상품(실사례 ``사이즈: 150（무몰딩）``)에서는 그 값이 담당자가 제품명 칸에 적는
#: 글자다. 이 값은 :func:`main_product_name` 으로 깎지 않는다 — 깎으면 꼬리 괄호가 날아가
#: ``150`` 만 남는다(``（무몰딩）`` 은 제품 사양이지 부가 설명이 아니다).
#: ``제품`` 키가 함께 있으면 그 칩이 **먼저** 나오므로 일괄 입력은 제품 키 값을 쓴다.
...
COPY_TARGET_BY_KEY: dict[str, str] = {
    ...
    **{key: "product_name" for key in SIZE_OPTION_KEYS},
