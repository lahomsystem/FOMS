...
#: ①③ 은 둘 다 `INTENT_CHANGED`(구매 의사 취소)로 보낸다 — 그 주문건에 대한 구매 의사가
#: 사라지는 것이 사실이고, 범례에 "금액 정정·재결제" 코드는 **없다**(11종 전수 확인).
#:
...
#: 뒷절이 사례 ③(재결제 목적)을 정확히 겨눈다. 그래서 `INTENT_CHANGED` 로 보내는 것은
#: **고객이 그 주문의 취소에 동의한 건에 한한다** — 판매자가 금액을 고치려고 임의로
#: 넣는 것이 아니다. 화면(반품 모달)이 이 조건을 사람에게 말하고, 상세 사유에 통화·동의
#: 근거를 남기게 한다. 동의 없는 건은 이 버튼이 아니라 판매자센터에서 사람이 처리한다.
#:
...
RETURN_REASONS = {
    "INTENT_CHANGED": "구매 의사 취소(변심·주문 취소·재결제)",
    "COLOR_AND_SIZE": "색상 및 사이즈 변경",
}
