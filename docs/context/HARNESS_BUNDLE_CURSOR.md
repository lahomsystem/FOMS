...
4. Claude Code session augmentation (CLAUDE.md) (`claude_md`)
...
- **cursor**: Primary IDE integration: .mdc rules apply natively; use Cursor browser MCP for manual web QA per project policy.
...
- **브라우저**: 탐색·수동 재현·디버깅은 **Cursor browser MCP**. 반복 가능한 QA·릴리스 스모크는 **gstack browse** 런타임 도입 시 예약(현재 미도입).
...
## 새 세션 시작 프로토콜
1. `docs/AI_STATUS.md` 읽기 → 50줄로 전체 상황 파악
2. **핵심 코어 변경(DB/Auth/API)** → RPI 프로토콜 필수:
