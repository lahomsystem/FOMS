...
Codex has no first-class browser tool in this harness; defer to Cursor browser MCP or manual steps documented in workflows.
...
5. Claude Code session augmentation (CLAUDE.md) (`claude_md`)
6. Harness engineering master plan (`plan_harness_engineering_master`)

### Additional runner notes

- **codex_cli**: Codex CLI should treat AGENTS.md as the portable baseline; include Cursor rules for parity with IDE agents.
- **verification**: Follow verify-result workflow expectations when claiming build or import verification complete.
