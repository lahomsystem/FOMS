# ERP 셸 하트비트 — 렌더 전에 끝나는 재검증 (설계, 승인 대기)

> 2026-09-01 작성. 상태: **설계만**. 구현 착수는 사용자 승인 후.
...
| 단계 | 내용 | 완료 기준 |
|---|---|---|
| S0 | 쓰기 경로 전수 조사(§5-1·5-2) | 표 1장: 테이블×쓰기경로×카운터 갱신 여부. 미갱신 0 이 아니면 대상 화면 축소 |
| S1 | 패밀리 카운터 `INCR` 배선(읽는 쪽 없음) | 계약: mutation 후 카운터 증가, Redis 없을 때 무해 |
