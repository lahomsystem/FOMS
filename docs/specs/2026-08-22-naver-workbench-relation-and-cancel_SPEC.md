...
| # | 결정 | 근거 |
|---|------|------|
| D1 | ~~발송처리 단독 호출은 **ADDON/REPAY 집에만** 연다.~~ → **2026-08-24 개정: ADDON 집에만** 연다. NEW·REPAY 는 발주확인이 먼저다. | 네이버 발송관리 화면은 발주확인 없이 발송처리를 받는다(사용자 실화면). 다만 NEW 를 열어 두면 실제 출고 전 오조작이 그대로 불가역 호출이 된다. **개정 사유**: 재결제(REPAY)에는 "물건이 따로 나가지 않는다"가 거짓이다 — 원 주문을 취소하고 그 물건값을 다시 낸 것이라 원 주문의 물건이 나중에 한 번 나간다. 붙이기 직후 파란 `지금 닫기`가 켜져 있어 두 번째 클릭이 그대로 불가역 호출(구매자에게 "배송 시작"·취소 버튼 소멸)이 됐다. `2026-08-19` 스펙 §3 원안(REPAY = 신규와 같게)으로 복귀. 원장: `docs/plans/2026-08-24-naver-d1-repay-revision-ledger.md` |
...
**2026-08-24 개정분이 반영된 표다**(D1 개정 — REPAY 가 NEW 쪽으로 내려왔다).

| 관계 | 발주확인 전 | 발주확인 후 |
|------|-------------|-------------|
| ADDON | **[지금 닫기]** 열림 (네이버가 발주확인을 함께 처리한다) | [지금 닫기] |
| REPAY | 회색 잠금 + 사유("재결제 집이라 발주확인이 먼저입니다") | [발송처리] + "실제 출고·시공 시점에만" 경고 |
| NEW | 회색 잠금 + 사유 | [발송처리] + "실제 출고·시공 시점에만" 경고 |

- 서버 가드(`dispatch_order`)도 같은 규칙이다: 집 **전체**가 `CLOSE_NOW_RELATIONS`
  (= `("ADDON",)`, `fulfillment.py`) 면 발주확인 전이어도 통과, `REPAY`·`NEW`(또는 빈값)가
  하나라도 섞이면 막고 사유를 적는다. 화면(`naver_ingest.py` 의 `close_now`)은 이 상수를
  **import 해서** 쓴다 — 튜플을 두 곳에 손으로 적으면 서버와 화면이 조용히 갈린다.
...
- **R1** 발송처리 단독 호출이 네이버에서 거절될 수 있다(문서에 자동 발주확인 명시 없음).
  → ADDON 으로만 한정했고(2026-08-24 개정 전에는 REPAY 포함), 실패는 사유와 함께 남는다.
  실호출 1건으로 사용자가 확인한다.
