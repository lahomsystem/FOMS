...
| 단계 이동 | `workflow.stage` | — (취소·보류 포함) |
| **주문 취소** | 휴지통 이동(`/delete/<id>`) | 요청 **전에** 받아 폼에 실어 보낸다 |
...
**축에서 빠진 것**: 실측일 · AS 방문일 · **제품 세부 내역**(규격·색상·손잡이·옵션·품목 추가삭제)
· 연락처 · 주소 · 비고. 제품 세부는 사용자 결정(2026-08-14)으로 **기록만** 한다 — 축에 넣으면
사유 창이 저장의 60% 에서 뜨는데(빼면 16%) 그 대부분이 손잡이·색상 같은 잦은 실무 수정이었다.
...
### 3.4a 주문 취소(휴지통 이동) 사유

저장과 달리 삭제는 **요청 전에** 받는다 — 삭제는 되돌리기가 비싸고, 요청이 나간 뒤에는
사용자가 목록 화면을 떠나 사유를 붙일 기회가 없다.

- 화면: 삭제 버튼 첫 클릭을 `order-delete-reason.js` 가 가로채 사유 시트(collect 모드)를 열고,
  고른 값을 `reason_code`/`reason_note` hidden 필드에 실어 폼을 제출한다. 시트를 닫으면
  **삭제하지 않는다**(사용자가 취소한 것으로 본다).
- 서버: `POST /delete/<order_id>` 가 `record_action_reason` 으로 같은 트랜잭션에 기록한다.
  change set 검증은 하지 않는다 — 삭제는 필드 변경을 남기지 않아 대조할 원장 행이 없다.
  사유가 없거나 형식이 틀리면 `unspecified` 로 남기고 **삭제는 진행한다**(fail-open).
- 감사 헤더: `ORDER_SOFT_DELETED` 의 `detail` 에 `change_set`·`reason_code`·`reason_note`.
...
| 3축(시공일·금액·제품 세부) + 최초 입력 제외 | 60% (하루 약 31회) |
| 〃, 제품 세부를 규격·품목명만으로 좁히면 | 42% (하루 약 22회) |
| **현행: 시공일·금액·단계 + 최초 입력 제외(2026-08-14)** | **15%** (하루 약 8회) |

현행 규칙의 첫 트리거는 **전부 금액**(15건)이다. 시공일·단계는 이 기간(2일) 0건이고, 제품
세부는 기록만 한다. 삭제(휴지통 이동) 사유는 저장 원장과 별개로 매 삭제마다 1건씩 쌓인다.
