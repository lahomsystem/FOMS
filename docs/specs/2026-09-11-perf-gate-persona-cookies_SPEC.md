...
### 1.3 예외/제약 조건
- ~~**커버리지 축소를 인정하고 기록한다.**~~ → **2026-09-11 닫힘.** `foms_ptr=fine` 이
  coarse 전용 표면을 게이트 밖으로 밀어낸 공백을, 터치 태블릿 페르소나(`tablet-coarse`,
  `foms_scr=1180`·`foms_ptr=coarse`) **두 번째 패스**로 닫았다. 바이트만 판정한다 —
  latency 예산은 CI 심판석 기준이라 페르소나마다 시드하면 심판석이 둘로 갈린다.
  실측으로 드러난 coarse 전용 표면의 무게(pc 대비): 시공 +64% · 완료 +42% · AS +40% ·
  도면창구 +26% · 생산 +20% · 대시보드 +12%. 적지 않은 몫이 안 잡히고 있었다.
- **예산 재시드는 CI 에서만.** `reconcile_seed_budget` 이 로컬 `--seed` 의 TTFB 예산 갱신을
  막는다(CI 심판석 값 보존). → **2026-09-11 자리 마련.** perf-gate 워크플로에
  `workflow_dispatch` 입력 `mode: seed` 를 넣어 CI 러너에서 `--seed` 를 돌릴 수 있게 했다.
  결과는 **커밋하지 않고 artifact 로만** 낸다 — 봇이 예산을 스스로 헐겁게 만들면 게이트가
  조용히 무력화되므로, 사람이 diff 를 읽고 반영한다.
