# FOMS 현재 상태
> 자동 업데이트: 2026-04-12 | 마지막 작업: wdcalculator renderEstimatesList view batch 완료
...
## 진행 중
- [2026-04-12] 구조 트랙 다음 단계: render list view까지 빠진 뒤 남은 giant inline cluster를 재감리한 결과, 다음 안전 후보는 `baseComponentsContainer` live interactions(add row + click/input/change delegation)이다. 현재는 이 base row event wiring의 DOM/selectors/recalc contract freeze를 준비 중이다.
...
## 알려진 이슈
- 차단 이슈 없음. render list view까지 빠지면서 남은 주요 리스크는 `refreshAfterSave`, `resetInputFormKeepCustomerName`, `loadEstimateToInputForm`, save button clone/replace, URL bootstrap, 그리고 `addEstimateBtn` 이중 wiring이 `editingEstimateId`, `currentDatabaseEstimateId`, sidebar refresh 타이밍과 더 촘촘히 결합돼 있다는 점이다. 다음 batch는 그보다 결합이 얇은 `baseComponentsContainer` live interactions만 분리하고, save/API/URL orchestration과 duplicated reset 흐름은 계속 host에 남겨야 한다.
