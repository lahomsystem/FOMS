# FOMS 현재 상태
> 자동 업데이트: 2026-04-12 | 마지막 작업: wdcalculator baseComponentsContainer live interactions batch 완료
...
## 최근 완료 (최대 5개)
- [2026-04-12] wdcalculator baseComponentsContainer live interactions batch 완료: `addBaseComponentBtn`와 `#baseComponentsContainer`의 click/input/change delegation을 기존 `static/js/wdcalculator/base-components-ui.js`로 흡수하고, host giant script에서는 `initBaseComponentsLiveInteractions()` bootstrap만 남겼다. 선행으로 고정한 Node contract는 add row, fee add/remove, mode toggle, 최소 1행 guard, manual 30cm→1cm auto-sync, pricing type change surface를 helper 기준으로 계속 검증하게 유지했다. 최종 검증은 related pytest `11 passed`, `APP_OK`, 신규 lint 없음이었다.
- [2026-04-12] wdcalculator renderEstimatesList view batch 완료: in-session estimates list view(`renderEstimatesList`, summary card, post-render style pass)를 `static/js/wdcalculator/render-estimates-list.js`로 분리하고, host giant script에는 `WdCalculatorRenderEstimatesList.configure({ getEstimates, formatNumber, escapeHtml, formatNotesText, onRenderComplete })` bridge만 남겼다. 선행으로 고정한 Node contract는 empty state, `data-estimate-id`/`.estimate-display-name` markup, 1건 vs 2건 summary layout, escaped option/notes text, save button 노출, render 후 `calculateTotalEstimates()` callback을 helper 기준으로 계속 검증하게 유지했다. `/wdcalculator` render contract에는 새 helper load-order를 추가했고 최종 검증은 related pytest `13 passed`, `APP_OK`, 신규 lint 없음이었다.
...
## 진행 중
- [2026-04-12] 구조 트랙 다음 단계: base row live interactions까지 빠진 뒤 남은 giant inline cluster를 재감리한 결과, 다음 안전 후보는 URL/deep-link bootstrap(`order_id` back link, `estimate_id` URL load, product-ready polling)이다. 현재는 이 URL bootstrap의 DOM/URL/async retry contract freeze를 준비 중이다.
...
## 알려진 이슈
- 차단 이슈 없음. base row live interactions까지 빠지면서 남은 주요 리스크는 `refreshAfterSave`, `resetInputFormKeepCustomerName`, `loadEstimateToInputForm`, save button clone/replace, URL bootstrap, 그리고 `addEstimateBtn` 이중 wiring이 `editingEstimateId`, `currentDatabaseEstimateId`, sidebar refresh 타이밍과 더 촘촘히 결합돼 있다는 점이다. 다음 batch는 그중에서도 상대적으로 얇은 URL/deep-link bootstrap만 분리하고, save/add/reset/list orchestration은 계속 host에 남겨야 한다.
...
| `static/js/wdcalculator/base-components-ui.js` | 복합 기본견적 행 렌더/DOM 읽기/product select 옵션 갱신과 add row/click/input/change live interactions를 giant inline script 밖으로 분리한 base-components module |
...
| `tests/test_wdcalculator_base_live_events_contract_node.py` | baseComponentsContainer live interactions(add row + click/input/change delegation) contract를 Node로 검증하는 focused regression test |
| `tests/support/wdcalculator_base_live_events_contract_node_checks.js` | base-components helper를 VM DOM stub에서 직접 실행해 add row/fee/mode/manual pricing live wiring contract를 고정하는 Node support script |
