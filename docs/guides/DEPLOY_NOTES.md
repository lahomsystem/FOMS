...
### 뭘 했나요?
- **작업 큐에서 "삭제(휴지통)" 선택 가능**  
  ERP 작업 큐에서 여러 주문 선택 후 상태를 **삭제(휴지통)** 로 바꾸면, 해당 주문들이 휴지통으로 이동합니다.
- **휴지통 영구 삭제가 정상 동작**  
  휴지통에서 "선택 영구 삭제", "모든 주문 영구 삭제" 시 FK 오류 없이 삭제되도록 수정했습니다.
- **사용자 삭제 시 채팅 FK 오류 해결**  
  관리자에서 사용자 삭제 시, 해당 사용자의 채팅 메시지·채팅방·접속 로그를 먼저 정리한 뒤 삭제하므로 FK 오류가 나지 않습니다.

### 사용자 입장에서는?
- 작업 큐에서 한 번에 여러 주문을 휴지통으로 보낼 수 있고, 휴지통에서 영구 삭제가 정상 동작합니다.
- 관리자 계정 관리에서 사용자 삭제가 채팅 이력이 있어도 정상 처리됩니다.
...
- **DB 관리 스크립트 비밀번호 환경변수화 (NEXT-004)**  
  `scripts/ops/db_admin.py`에서 관리자(admin) 계정 기본 비밀번호를 **환경변수**로 바꿨습니다.
