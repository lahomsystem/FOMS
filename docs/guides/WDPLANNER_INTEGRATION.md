...
```powershell
.\scripts\maintenance\build_wdplanner.bat
```
...
1. `scripts\maintenance\build_wdplanner.bat` 실행
...
│       ├── assets/
│       └── ...
├── scripts/maintenance/build_wdplanner.bat  # 빌드 스크립트
└── Add In Program/
    └── WDPlanner/                   # 원본 소스 코드
        ├── src/
        ├── package.json
...
2. `scripts\maintenance\build_wdplanner.bat` 실행
3. 브라우저 캐시 삭제 후 새로고침
...
빌드 및 배포는 `scripts/maintenance/build_wdplanner.bat` 스크립트를 사용하세요.
