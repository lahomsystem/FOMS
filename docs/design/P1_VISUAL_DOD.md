...
- [ ] P1 시각 파일(`foms-shell.css`, `dashboard_mobile_v2_body`, queue card v2, detail partials) **deploy 브랜치 커밋·push**
- [ ] Railway: `ERP_MOBILE_V2_ENABLED=true`, `FOMS_V3_SHELL_COHORT=all` (또는 파일럿 id)
- [ ] `scripts/ops/verify_mobile_v2_rollout.ps1` PASS

### B2. Mockup 4종 (REDESIGN §6)

| Mockup | DOM/IA 체크 | Visual (390×844) |
|---|---|---|
| mobile-home-dashboard | chip-strip, section-header, queue-card, FAB, sort chips | Playwright baseline vs `docs/design/mockups/` |
| mobile-order-detail | hero, quick×4, KV, attach-grid, timeline, sticky CTA | 동일 |
...
| C07 | attach grid on mobile detail |
| C14 | wizard step2 product accordion |
