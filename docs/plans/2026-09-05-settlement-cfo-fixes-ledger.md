...
| P push → CI → 스테이징 QA(+EXPLAIN 인덱스 채택) → 운영 승격 | IN PROGRESS — push_own 4커밋 → 원격 deploy `dc02e37bf`(코드 `826022874`). **CI 4/4 green**. 스테이징 QA: API 10/10 PASS(`scratchpad/qa_v15_api_result.json` — exception_totals 9키 total 438·AMOUNT_DIFF 2 · strip 436 = total − AMOUNT_DIFF(스트립 제외 계약 실증) · 행 라벨 "정산액≠출고가"·링크 `/erp/orders/4485`·ref 5키(정산 587,880 vs 출고가 1,191,900, case 5건) · B-01·F-02 문구 리터럴 · operations.js 두 원값 문구) · **EXPLAIN 실증**(`qa_v15_explain_result.json`): 스테이징 alembic `naversettle_02`, 인덱스 2종 생성, case 30일 0.23ms·1년 2.73ms·commission 30일 0.44ms·1년 5.21ms **전부 Index Scan 채택**(감사 시점 Seq Scan 1.8~5.9ms) · 실화면(핀 6줄 20260906b 도달, 확정 구간 부제, 예외 큐 AMOUNT_DIFF 2행 + [열기], 콘솔 0·네트워크 실패 0, `qa_v15_exceptions.png`). rows API 네이버 칸은 스테이징 모집단에 네이버 링크 주문이 0건이라 계약 테스트로 판정. **운영 승격 완료 — PR #301 → production `2fce6197c`**(승격 트리 게이트: APP_OK·heads 단일 `naversettle_02`·settlement 997·306·smoke PASSED, PR 검사 4종 SUCCESS). 운영 표 규모 `naver_settle_case` 4,277행 6MB·`commission` 9,508행 13MB — 인덱스 생성 락 밀리초. 상태 DONE |
