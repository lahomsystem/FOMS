...
| T12 | 요약 탭 크로스 스트립(W1-A 커널 → W2-A 프론트 → 셸 앵커) | agent | 계약서 §8.1 ①~⑤ | W1-A DONE 083070b78(쿼리 5, HTTP 절은 STRIP_VIEW 등장 시 자동 활성) · W2-A PENDING (착수 HEAD 0a54b25b5) |
| T13 | 실무 탭 네이버 정산 컬럼 + 정산상태→차감청구 + naversettle_01 인덱스(W1-C → W2-B) | agent | 계약서 §8.1 ①~⑦ | W1-C DONE 476768ef3(naver_settlement{status,settle_expect_date,settle_complete_date,amount}, amount 는 화면 미렌더) · W2-B DONE(커밋 직후 SHA는 git log 참조; 셸 핀 4줄 20260902b→20260903a 범프는 총괄 Wave 3) (착수 HEAD 0a54b25b5) |
| T14 | CSV 내보내기 5종 + 감사 라벨(W1-B 커널 → W2-A UI) | agent | 계약서 §8.1 ①~⑤ | W1-B DONE df734cc46(110칸, FILTER_FIELDS daily 계열은 type/q 불가) · W2-A RUNNING (착수 HEAD 0a54b25b5) |
