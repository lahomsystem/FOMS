...
| 것 | 운영 |
|---|---|
| 결과 UI(완료/일부/실패/대기 4상태 · 진행 폴링 · 실패 줄마다 재시도) | PR #219 `376362c3` |
| 안 붙은 수집분 짚기 + 수령인명 축 | PR #227 `d65e4030` |

운영 코드 표식 전수 확인: `build_day_summary`·`find_unlinked_matches`·`bulk-dispatch/state`·
`data-naver-dispatch-retry`·`wb-bulkdispatch__linkline`·`수령인명 일치` 모두 존재.

### 승격이 막혔던 자리 (기록)

- 승격 PR 전부가 perf-gate 에 막혔다. perf-gate 는 **스테이징을 재지만 예산 파일은 PR 트리에서
  읽는다** — 예산 재시드가 deploy 에만 있어 승격 트리는 옛 예산으로 판정했다. 타 세션이
  `303b53b3` 로 운영 계보에 반입한 뒤 해소.
- 그 전에 `/erp/completion` 프래그먼트 증가를 **회귀로 오판**해 전송 상한을 거는 수정을 만들었다가
  버렸다. 실측은 바이트가 는 게 아니라 **옮겨온 것**이었다(프래그먼트 13,661+API 12,456 = 26,117 →
  프래그먼트 26,109+API 0 = 26,109, 요청 2→1). **프래그먼트 숫자만 보고 사라진 API 호출을 안 셌다.**
- `git add -A` 가 타 세션 변경을 삼킨 커밋 1건 — 되돌리고 pathspec 으로 재작성.

### 해소됨 — 운영 워크벤치 자산 핀 (2026-09-01)

운영 `naver_workbench.html` 핀이 `?v=20260831e` 인데 그 뒤에 CSS 규칙 2개
(`wb-bulkdispatch__linkline`·`__linklist`)가 같은 핀으로 올라갔다. SW staticCacheFirst 라
**이미 워크벤치를 연 적 있는 사용자는 옛 CSS 를 계속 쓴다** — '붙이면 대상이 되는 집' 줄이
주황 배경 없이 맨 텍스트로 뜬다(읽히기는 한다). deploy 는 `20260901a`(타 세션 커밋이 올림).
**해소**: 타 세션이 PR #229 를 승격하며 핀을 함께 올렸다(`7976fb2c`). 이 세션이 만진 5개 파일이 deploy 와 production 에서 **바이트 동일**임을 확인했다 — 따로 고칠 것이 없다.

### 여전히 미검증

실브라우저 폴링·진행 문구. 누르는 순간이 불가역이라 실호출을 하지 않았다.

## T9 — 지난 날짜 띠 + **더 큰 사실 발견** (2026-09-01)

### 지난 날짜도 보여준다

날짜가 다르면 띠를 통째로 감추던 규율이 **지난 날 빠진 건까지 감췄다.** 값은 보는 날짜로
채우고 **불가역 조작만 오늘로 잠근다**(`is_today`). 감춰야 할 것은 날짜가 아니라 오늘 것을
다른 날 화면에 얹는 일이었다. deploy `bdff05c8`, CI green.

### 사용자 정정이 드러낸 것 — 붙이기 문제가 아니다

내가 "오늘 실측 링크 없는 9건은 네이버 유래가 아니다(예약금·전화 접수)"라고 보고했는데
사용자가 **"소수 제외 다 네이버 실제 주문"** 이라고 정정했다. 운영 재조회 결과 내 판정이
틀렸고, 원인은 매칭 축이 아니라 **수집 범위**였다:

- 그 9건은 이름으로도 전화로도 `external_order_links` **전체에서 0건**이다(붙은 것 포함).
  즉 **수집분 자체가 FOMS 에 없다.**
- 수집 첫 실행 `2026-08-25 00:44`, 수집분이 말하는 **가장 오래된 네이버 주문일 `2026-08-24 10:29`**.
  총 215행 / 63묶음뿐이다.
- 접수일 08-03·08-04·08-13·08-18·08-22 5건은 명백히 수집 시작 전이고, 08-25~09-01 접수분도
  네이버 주문일이 08-24 이전이면 같은 이유로 없다.

**결론: 이 건들은 붙이기로 해결되지 않는다.** 원본이 없어 발송 대상이 될 수 없고, 띠도
짚을 수 없다(짚을 수집분이 없다). 필요한 것은 **과거 주문 소급 수집(백필)** 이다 —
커머스API 조회 기간 제한 안에서 워터마크를 과거로 한 번 돌리는 일. 미착수.

곁가지로, 화면은 지금 "네이버 원본이 없는 건"과 "네이버 주문이 아닌 건"을 **구별하지
못한다** — 둘 다 링크가 없다. `structured_data['source']` 는 오염분이 있어 축이 못 된다(§4.2).
