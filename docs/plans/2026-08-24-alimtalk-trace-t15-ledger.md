...
| T15.3 | 이력 API — `GET /api/orders/<id>/events` 에 `event_type` 필터 + `created_by_name` + 한글 `label` 추가(읽기 전용·additive) | `pytest tests/domains/test_order_events_api.py -q` PASS(신규 파일) + 기존 events 테스트 PASS + APP_OK | DONE | (이 커밋) | 25 passed. 이름은 id 집합 1회 조회로 붙임(N+1 금지) |
