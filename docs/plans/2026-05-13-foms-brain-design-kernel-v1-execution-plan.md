...
> 작성일: 2026-05-13 | 상태: 🟡 승인대기 | 성격: FOMS Brain 진짜 제품화 후속 실행 계획
...
Add In Program/FOMSBrainDesigner/src/
  domain/
    ontologyTypes.ts              # Atomic Ontology TS 타입
    componentCatalog.ts           # box/panel/door/shelf/ep/sr/base/hardware 정의
    formulaEngine.ts              # formula evaluate/recalculate
    constraintEngine.ts           # frontend-side fast validation
    designCommands.ts             # command/patch 타입과 deterministic executor
    assemblyFactories.ts          # built-in wardrobe generator
    correctionDelta.ts            # before/after delta 생성
  stores/
    designerStore.ts              # DesignJson -> Assembly graph 기반으로 확장
  canvas/
    CabinetScene.tsx              # component graph 렌더링
    SelectionGizmo.tsx            # gizmo target_uuid 중심으로 정리
    DimensionLines.tsx            # assembly/module/component 치수 표시
  ui/
    InspectorPanel.tsx            # selected component editor
    ModulePanel.tsx               # module count, door type, EP/SR 편집
    ComponentTreePanel.tsx        # UUID tree/list
    CommandPanel.tsx              # DesignCommand JSON / LUI skeleton
    ValidationPanel.tsx           # constraint 결과 상세 표시
```
...
foms/services/designer/
  ontology_types.py               # backend canonical schema
  component_catalog.py            # component/material/rule catalog
  formula_engine.py               # deterministic formula evaluator
  constraint_engine.py            # hard validator v2
  assembly_factories.py           # wardrobe/kitchen/shoe-rack starter factories
  command_engine.py               # DesignCommand -> patch
  corrections.py                  # correction delta 기록 강화
  validator.py                    # existing MVP validator를 constraint_engine으로 위임
  bom.py                          # component graph 기반 BOM v1

foms/api/designer/
  commands.py                     # POST /api/designer/commands/preview|apply
  components.py                   # optional component catalog endpoint
  validation.py                   # detailed constraint result 반환
  projects.py                     # version 저장 시 assembly graph 저장

foms/persistence/designer/
  models.py                       # 필요 시 schema version/correction delta 확장
  repositories.py                 # graph/correction query 추가
```
...
tests/domains/test_designer_frontend_contract.py
```
...
- [ ] 현재 `FOMSBrainDesigner` data shape, store, canvas, API client 확인
- [ ] 현재 backend `validator.py`, `defaults.py`, `projects.py`, `ai_runs.py` 확인
- [ ] DB `designer_project_versions.design_json` 호환 방식 확인
- [ ] 현재 UI에서 실제로 불가능한 작업 inventory 작성

검증:

- [ ] “현재 shell vs 필요한 kernel gap” 요약을 run record에 기록
...
- [ ] Assembly/Module/Component/Material/Formula/Constraint 타입 정의
- [ ] component kind catalog 정의 (`box`, `panel`, `door`, `shelf`, `drawer`, `ep`, `sr`, `base`, `hardware`, `cutout`)
- [ ] material catalog seed 정의 (`PB_18T`, `MDF_18T`, `PET_DOOR`, `HARDWARE_RAIL`)
- [ ] JSON schema version 2 고정

검증:

- [ ] TS typecheck 통과
- [ ] Python import/app smoke 통과
- [ ] schema fixture round-trip 테스트 통과
...
- [ ] formula expression 최소 DSL 정의
- [ ] `assembly.width`, `module.width`, `component.dimensions.*` 참조 가능
- [ ] parent dimension 변경 시 child formula 재계산
- [ ] 계산 결과를 mm 정수로 normalize
- [ ] circular formula 감지

검증:

- [ ] `door_height = total_height - top_sr - base - gap` 테스트
- [ ] `module_width = (outer_width - ep_left - ep_right) / module_count` 테스트
...
- [ ] 기존 W/H/D validator를 constraint engine으로 확장
- [ ] 외경 합산 rule 구현
- [ ] parent boundary rule 구현
- [ ] 자재 최대 규격 rule 구현
- [ ] door gap / panel thickness rule 구현
- [ ] error/warning/info severity 분리

검증:

- [ ] `outer_width != ep + module_sum` 실패
- [ ] component가 parent 밖으로 나가면 실패
- [ ] 판재 최대 규격 초과 실패
- [ ] 기존 tests 유지
...
- [ ] base, side panels, top/bottom panels, back panel 생성
- [ ] module count에 따른 내부 box 생성
- [ ] 기본 shelf/door 생성
- [ ] `default_design_json()`을 schema v2 assembly로 전환하거나 dual-read 지원

검증:

- [ ] 3000W/2400H/620D/3-module fixture 생성
- [ ] component UUID 중복 없음
- [ ] validator 통과
...
- [ ] store의 `design`을 schema v2 assembly graph로 교체
- [ ] component kind별 렌더링 함수 분리
- [ ] panel/door/shelf/ep/sr/base 렌더링
- [ ] 선택 대상은 component UUID로 통일
- [ ] dimension line을 assembly/module/component 기준으로 표시

검증:

- [ ] `npm run build`
- [ ] 기본 wardrobe가 박스 하나가 아니라 부품 구조로 보임
- [ ] component 선택 시 Inspector에 정확한 UUID/role 표시
...
- [ ] module count 변경 UI
- [ ] door type 선택 UI (`sliding|swing|open`)
- [ ] EP/SR 값 편집 UI
- [ ] selected component property editor
- [ ] component tree/list 표시
- [ ] 변경 시 formula 재계산 + validator 결과 즉시 표시

검증:

- [ ] 2통/3통/4통 변경 시 3D 구조 재생성
- [ ] EP/SR 수정 시 관련 치수 재계산
- [ ] invalid 값 입력 시 저장 차단
...
- [ ] `DesignCommand` 타입 고정
- [ ] deterministic command executor 구현
- [ ] `move_component`, `resize_component`, `set_property`, `generate_layout` 구현
- [ ] preview/apply 분리
- [ ] current selection context를 command target으로 사용
- [ ] API: `POST /api/designer/commands/preview`, `POST /api/designer/commands/apply`

검증:

- [ ] “shelf-003 y +50” command preview
- [ ] invalid command는 저장/적용되지 않음
- [ ] apply 후 correction delta 생성
...
- [ ] correction delta shape 강화
- [ ] target_id, before, after, reason, source, validated 저장
- [ ] rule candidate hint 저장
- [ ] fake embedding mode에서 correction text 저장
- [ ] 향후 pgvector retrieval 기준 metadata 설계

검증:

- [ ] component property 수정 시 delta 생성
- [ ] command apply 시 delta 생성
...
- [ ] 기존 designer API tests 통과
- [ ] schema v1 fixture load 가능
- [ ] schema v2 fixture save 가능
...
- [ ] 구현 파일 inventory 기록
- [ ] schema v2 contract 기록
- [ ] 검증 명령/결과 기록
- [ ] 다음 계획: Vision-to-Ontology, LUI real parser, BOM/DXF
...
- [ ] 외경 합산/parent boundary/자재 최대치/door gap rule이 validator에서 검증된다.
- [ ] invalid design은 저장되지 않는다.
