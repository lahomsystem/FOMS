...
## 운영 실화면 대조 (2026-08-31 — 별 세션, 발송·쓰기 없음)

승격은 PR #200 로 끝났고(production `639feabe`), 오늘 운영 이력 탭을 직접 열어 대조했다.
`claude_master`(id 57) 해제 → `GET /admin/naver-ingest/triage?tab=all` → 재잠금(오라클 200).

운영 HTML 50행 실측:

| 본 것 | 수 |
|---|---|
| 상태 칸 래퍼 `wb-st-wrap` | 50(전 행) |
| 축 라벨 `FOMS` / `네이버` | 50 / 50 |
| 축 라벨 `취소·반품` | **9** — 예외 줄이 값 있을 때만 나온다는 규칙이 실데이터에서 그대로 작동 |
| 값 표본 | `신규 결제` 38 · `주문 만듦` 36 · `받아옴` 13 · `재결제 →` 9 · `추가결제 →` 3 · **`취소 완료 08-28`/`08-27`** · `네이버 처리 없음` 2 |
| 자산 핀 | `naver-workbench.{css,js}?v=20260830c` |
| 찾기 칸 `wb-find` | 존재 |

즉 관계 축(재결제·추가결제)·취소 확정 날짜·예외 줄 조건부 렌더·`네이버 처리 없음` 예외 칸이
**운영 실데이터에서 전부 확인**됐다. 옛 단일 상태 칸 마크업은 0건.

## 20. 운영 실화면 지적 2건 반영 (2026-08-31)

사용자가 운영 화면을 직접 보고 지적했다. 계약 개정은 **계약서 §16**(그 절이 §15 보다 우선).

### 20.1 끝난 칸 두 개가 같은 색 → 발주확인=파랑 · 발송=초록

- `templates/admin/naver_workbench.html` — 파이프 칸에 축 표식 `--place`/`--ship` 추가.
- `static/css/admin/naver-workbench.css` — `.wb-pipe__s--place.wb-pipe__s--done` 를 `--wb-accent`(파랑)로.
  순서 선택자(`:first-child`)를 안 쓴 이유는 계약 §16.1 에 적었다.
- 계약 테스트 신설 `test_two_finished_steps_are_not_the_same_colour` — 두 표식의 동시 존재를 잠근다.

### 20.2 발송 꼬리 → `{시각} · ERP발송` / `{시각} · 판매자센터`

- `_history_pipe_note` 2·3번 줄 교체. **시각도 주체의 것**을 쓴다(우리 발송이면 `ours_at`).
  옛 코드는 두 경우 모두 네이버 `sendDate` 를 찍었다.
- 테스트 2종으로 갈라 잠갔다: `test_our_dispatch_says_erp_with_our_own_time`(KST 변환까지 —
  UTC naive `11:40` → 화면 `20:40`) · `test_seller_center_dispatch_says_so_with_naver_time`.

### 20.3 `네이버 기록 없음` — 사용자 결정으로 **유지**

지우자는 안이 있었으나(방금 발송한 집은 다음 수집 전까지 항상 이 모양) 사용자가 "현재 그대로
유지"를 골랐다. 판정·배지·경고 줄 모두 무변경. 근거와 되살릴 때 볼 것은 계약 §16.3.

### 20.4 자산 핀

CSS 를 고쳤으므로 `?v=20260830c` → **`20260831b`** (템플릿 2줄 + 핀 잠근 테스트 2곳 = 4자리).

**`a` 를 건너뛴 이유**: 타 세션이 같은 날 `20260831a` 를 먼저 썼다(`bfab9a11` — 그쪽도
CSS·JS 를 고쳤다). 같은 핀을 재사용하면 그 배포로 이미 `?v=20260831a` 를 캐시한 브라우저·
서비스워커가 **내 CSS 변경을 영영 안 받는다**(staticCacheFirst). 리베이스로 핀 줄이
저절로 맞아 diff 가 사라지는 모양이라 조용히 지나칠 뻔했다 — 핀은 diff 가 아니라
**값의 유일성**으로 본다.

검증: `tests/services/integrations/` **1084 passed** · `APP_OK` · `pre_push_smoke` exit 0.


## 21. 색·문구 운영 승격 완료 (2026-08-31)

- 승격 PR: **#203 → #204(머지)** — `534b6052`. #203 은 머지 직전 운영이 `#202` 로 앞서
  자산 핀이 충돌해 접었다. **오늘만 운영이 다섯 번 앞섰다**(#197·#196·#201·#202 + 내 #200).
  충돌은 매번 자산 핀 아니면 생성물 인벤토리였다 — 코드 의존은 한 번도 없었다.
- 담은 것: 파이프 색 구분(§20.1) · 발송 꼬리(§20.2) · 처리 탭 낱말 통일(§19) + 인벤토리 재생성.
  문서(원장·계약서)는 deploy 계보에 두고 **코드만** 올렸다.
- 승격 트리 검증(리베이스 후 재실행): `APP_OK` · `alembic heads` 단일 `naverdisp_00` ·
  `tests/services/integrations` + perf guard **1100 passed** · `pre_push_smoke` exit 0.
  PR 체크 4종(test·pg-lane·harness·perf-gate) 전부 pass.

### 21.1 운영 실화면 확인 (완료 정의 충족)

`claude_master`(id 57) 해제 → 1회 조회 → 재잠금(로그인 200 오라클). 자산 핀 `20260831b`.

| 본 것 | 수 |
|---|---|
| `wb-pipe__s--place wb-pipe__s--done`(파랑 발주확인 완료) | **48** |
| `wb-pipe__s--ship wb-pipe__s--done`(초록 발송처리 완료) | **23** |
| `· ERP발송` 꼬리 | **19** |
| `· 판매자센터` 꼬리 | **4** |
| 옛 문구 `네이버 확인됨` / `판매자센터에서 직접` | **0 / 0** |
| 처리 탭 옛 배지 `발주확인 전` | **0** (새 낱말만) |

실데이터 표본: `재결제 → #4639` / `발주확인 완료 2/2` / `발송처리 완료 2/2` /
`2026-08-31 09:47 · ERP발송`. 사용자 스크린샷에서 `네이버 기록 없음` 이던 그 09:47 건은
그 사이 수집이 한 번 더 돌아 네이버 `sendDate` 가 들어오면서 스스로 완료로 바뀌었다 —
그 배지가 대부분 사고가 아니라 **낡은 스냅샷**이라는 §16.3 의 판단을 실물이 뒷받침한다.
