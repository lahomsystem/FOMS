...
### §12.1 효과 확인 (스테이징 `87d73ea6b`, 같은 경로 6회 × 두 탭)

```
이력 탭  wb_template = 34 / 9 / 11 / 10 / 35 / 10          (전: 173~777, 널뛰기)
처리 탭  wb_template = 24 / 29 / 52 / 21 / 23 / 26
        (그 안) pane =  1 /  1 /  1 /  0 /  0 /  1          (전: 25~420)
```

**12회 연속 요청에 550ms 급 널뛰기가 한 번도 없다.** 워밍 전에는 측정마다 한두 번 나왔다.

덧붙여 드러난 것: 처리 탭 `pane` 195~420ms 도 **렌더가 아니라 컴파일**이었다. 워밍 뒤 0~1ms 다.
운영에서 세 번 중 두 번 195ms 가 나온 것은 요청이 서로 다른 프로세스에 붙어 각자 한 번씩
컴파일했기 때문이다.

총 render 는 크게 안 줄었다(576~905ms) — 남은 것은 전부 조회다. §11 의 후보 2~5 가 그것이고,
그중 pane 렌더는 이제 목록에서 빠진다.

---

## §13 고친 것 2 — 유령 주문 띠 168ms (GHOST-THIN-01, 2026-09-13)

§11 후보 3번. 처리 탭에서 단일 최대 항목이었다.

### 먼저 쟀다 — 비용은 파싱이 아니라 **읽어 오는 양**이다

운영 DB 읽기 전용 조회로 같은 489행을 두 방식으로 가져와 비교했다(같은 연결·연속 실행).

| 방식 | payload | 조회 | 파이썬 판정 |
|---|---|---|---|
| 통째 스냅샷 | 1,958KB | 788ms | 1.9ms |
| 판정 경로만 투영 | **290KB** | **167ms** | 2.0ms |

`find_ghost_orders` 는 주문에 붙은 링크를 **전부** 읽는다(운영 2,388개 중 주문 붙은 것 489개,
주문 104건). 그런데 판정이 보는 것은 클레임 블록과 결제 금액뿐이고, 제품명·옵션·주소·배송 이력은
한 술어도 읽지 않는다. **파이썬은 어느 쪽이든 2ms** 다 — 고칠 자리는 조회다.

### 고침 — 술어가 아니라 입력만 얇게

`ghost_orders._ghost_snapshot_projection(session)` 을 추가해 SQL 이 축소 문서를 조립하고,
그 문서를 `raw_snapshot` 자리에 그대로 넣는다. `_claim_of`·`_fold_link` 는 **한 줄도 안 바뀐다**.
같은 규율의 선례가 `naver_ingest._snapshot_projection` 이고, 블록 키 목록은 `mapping` 에서
파생한다(손으로 적으면 이름이 늘 때 얇은 경로만 옛 목록으로 남는다 — R-7 이 그 사고였다).

**금액에는 COALESCE 를 쓰지 않는다.** 처음엔 최상위 폴백을 넣었는데, `_fold_link` 는
`productOrder` 안만 보므로 평평한 응답에서 얇은 경로가 **더 큰 금액**을 냈다. 계약 테스트가
그 차이를 잡았고, 동치를 택했다(폴백을 넣는 것은 별개 판단이다).

### 증명

- 운영 실데이터 489행 전량 대조: 판정 불일치 **0건**, 금액 불일치 **0건**(클레임 있는 행 119개 포함).
- `tests/services/integrations/test_naver_ghost_thin_projection.py` — 모양 8종의 판정·버킷 동치,
  블록 목록이 `mapping` 에서 파생되는지, **음성 대조군**(모양에 클레임 있는 것과 없는 것이 둘 다).
- `tests/postgres/test_naver_ghost_thin_projection_pg.py` — SQLite 레인은 투영이 폴백이라 이 축을
  못 증명한다. PG 레인에서 모양별 버킷 동치 + SQL 에 `raw_snapshot AS` 본문이 없음 + 띠 내용
  (전부 취소만, 부분 취소 제외, 금액 합) 동치. 로컬 PG 17 격리 클러스터에서 760건 전량 green.
- 돌연변이: 투영을 원본 컬럼으로 되돌리면 PG 계약이 red.

### §13.1 스테이징 전후는 **판정 불가** (정직하게 기록)

같은 경로 5회씩 다시 쟀는데 스테이징 잡음이 효과보다 크다.

```
work  ghosts = 154 / 126 / 136 / 129 / 153      (전: 71~231)
      work_groups = 805 / 625 / 562 / 845 / 608 (전: 389~549 — 이쪽이 더 느려졌다)
```

`wb_work_groups` 가 전보다 느려진 것에서 알 수 있듯 **스테이징 부하·데이터가 그 사이 바뀌었다**.
이 조건에서 ghosts 숫자 차이를 효과라고 부를 수 없다. 이 수정의 증거는 스테이징 왕복이 아니라
**같은 연결에서 같은 489행을 두 방식으로 가져온 §13 의 표**(1,958KB·788ms 대 290KB·167ms)다.

운영 확인은 다음 1회 세션에서 `wb_ghosts` 를 읽어 판정한다.

---

## §14 남은 몫의 **공통 원인** — 같은 JSONB 를 한 요청에서 네 번 detoast 한다

운영 `external_order_links` 실측(2026-09-13):
...
| 본체(heap) | 1,992kB |
| **TOAST** | **10,184kB** |
| `raw_snapshot` 평균 | **2,194 bytes** |
| 링크 수 | 2,388 |

평균 2,194 bytes 는 PostgreSQL TOAST 임계(약 2KB)를 넘는다 — 즉 **대부분의 스냅샷이 본체 밖에
있고, 그 컬럼을 건드리는 조회는 행마다 TOAST 를 한 번 더 읽는다.**

한 번의 페이지 요청이 이 테이블을 **서로 다른 투영으로 네 번** 훑는다.

| 구간 | 훑는 모집단 | 운영 ms |
|---|---|---|
| `wb_work_groups` | 확인 대기 링크 + 형제 | 112~159 |
| `wb_ghosts` | 주문 붙은 링크 489 (§13 에서 축소) | 168 → ? |
| `wb_refresh` | 채널 전체 링크 **2,388** | 73~97 |
| `wb_history` | 이력 한 쪽(50집) | 99~145 |

`wb_refresh` 의 SQL 을 운영에서 `EXPLAIN (ANALYZE, BUFFERS)` 하면 이렇게 나온다.

```
Seq Scan on external_order_links (actual time=0.116..50.292 rows=2388 loops=1)
  Buffers: shared hit=14736
Execution Time: 50.519 ms
```

2,388행에 **버퍼 14,736** — 행당 6페이지다. 스칼라 두 개만 뽑는데도 스냅샷 전체를 detoast 하기
때문이다. 이미 스칼라 투영을 쓰고 있으므로 **투영으로는 더 줄지 않는다.**

### 다음 갈래 (사용자 판단 필요 — 코어 변경은 Spec 먼저)

| # | 길 | 얻는 것 | 치르는 것 |
|---|---|---|---|
| A | 파생 컬럼 신설(`claim_status`·`claim_type`·`product_order_status`·`payment_amount`)을 수집 시점에 채우고 백필 | 네 경로 모두 TOAST 를 안 읽는다. 근본 원인 | 마이그레이션 + 백필 + 수집 경로 수정. 코어 변경이라 Spec·승인 필요 |
| B | ADMIN 전용 버튼 수(`wb_refresh`)를 짧은 TTL 로 캐시 | 73~97ms 를 대부분의 요청에서 없앤다 | 버튼 숫자가 최대 TTL 만큼 낡는다 |
| C | 버튼 수를 첫 화면 임계경로 밖으로(뱃지와 같은 수) | 같음, 낡지 않는다 | 요청 하나 늘고 버튼이 늦게 채워진다 |
| D | 요청 스코프 1회 조회를 세 경로가 공유 | 중복 detoast 제거 | 모집단·투영이 달라 합치는 설계가 필요하다 |
...
### §15.1 스테이징 효과 — 같은 창에서 전후 (2026-09-13)

`aac966c72` 배포 뒤 **백필 전** 5회, **백필 후**(2,427행 전량, 클레임 있는 행 603) 5회.
같은 스크립트·같은 계정·같은 시간대다.

| 구간 | 백필 전 | 백필 후 |
|---|---|---|
| `wb_ghosts`(처리 탭) | 126~154 | **59~68** |
| `wb_refresh` | 84~123 | **13~20** |
| `wb_pane_ctx` | 62~108 | 25~39 |
| `wb_history` | 126~153 | 53~63 |
| `wb_work_groups` | 562~1,070 | 315~485 |
| 서버 render 총 | 941~1,505 | **422~634** |

**손댄 두 구간은 2~6배 빨라졌다.** `wb_refresh` 는 사본 컬럼만 읽으므로 근거가 §14 의
"버퍼 14,736 → 249" 와 같은 축이다.

**손대지 않은 구간도 같이 내려간 것은 단정하지 않는다.** 그럴 법한 설명은 TOAST 를 덜 읽으니
shared buffer 경합이 줄었다는 것이지만, 스테이징 부하는 §13.1 에서 본 것처럼 시간대에 따라
크게 흔들린다. 확정은 운영에서 한다.

### 배포된 것과 남은 것

* 마이그레이션은 **Railway 배포가 자동으로 적용한다** — 스테이징이 배포 직후 `alembic current`
  에서 `nvmirror_00 (head)` 였다(사람이 돌리지 않았다). 운영도 같은 경로다.
* **백필은 손으로 돌려야 한다.** 안 돌려도 답은 같고(폴백) 비용만 예전 값이다.
* 운영 승격과 운영 백필은 사용자 요청을 받아서 한다.

---

## §16 운영 백필 완료 + 최종 실측 (2026-09-13, production `22acd17cc`)

사용자 승인으로 운영 백필을 돌렸다. `backfill_link_claim_mirror.py` — **대상 2,388행 · 채움
2,388 · 원본없음 0 · 계산불가 0**. 실행 중 수집된 5건은 수집 경로가 이미 사본을 채워 결과가
2,393행 전량이다(클레임 있는 행 583).

**전수 대조**: 운영 2,393행의 사본을 스냅샷에서 다시 계산해 비교 — **불일치 0건**.

사본만 읽는 스캔(다시 읽기 판정의 그 쿼리):

```
Seq Scan on external_order_links (actual time=0.013..1.376 rows=2393)
  Buffers: shared hit=249        Execution Time: 1.503 ms      (전: 14,736 / 50.519 ms)
```

### 운영 전후 (같은 스크립트·같은 계정·3회씩)

**이력 탭(`?tab=all`)**

| 구간 | 09-12 (수정 전) | 09-13 (수정 후) |
|---|---|---|
| `wb_work_groups` | 123~141 | **82~95** |
| `wb_history` | 99~145 | **68~92** |
| `wb_refresh` | 73~92 | **16~34** |
| `wb_pane_ctx` | 61~102 | **5~10** |
| `wb_template` | 9 / 551 / 594 | **7~49**(컴파일 스파이크 없음) |
| **서버 render** | **388~1,061** | **189~297** |

**처리 탭(`?tab=work`)**

| 구간 | 09-12 | 09-13 |
|---|---|---|
| `wb_ghosts` | 162~184 | **68~72** |
| `wb_pane_ctx` | 91~108 | **3~6** |
| `wb_bulk_dispatch` | 52~71 | 11~15 |
| `wb_refresh` | 80~97 | **17~21** |
| `wb_template`(그 안 `pane`) | 200~783 (195~420) | **3~40 (0)** |
| **서버 render** | **696~1,428** | **193~259** |

즉 **두 탭 모두 서버 렌더가 2~5배 빨라졌다.** 사용자 신고("두 탭 왕복이 오래 걸린다")의 서버
몫은 이것으로 닫힌다.
...
| 항목 | 값 | 성격 |
|---|---|---|
| `/erp/dashboard` render | 516ms | 대시보드 자체(슬라이스 캐시). 구간 계측이 아직 없다 |
| `wb_work_groups` | 82~95ms | 주소·제품명까지 읽어 사본 4개로는 안 끝난다 |
| `wb_ghosts` | 68~72ms | 투영까지 적용한 값. 더 줄이려면 모집단을 좁혀야 한다 |

세 항목 다 **사람이 기다리는 체감을 더 바꾸지는 않는다** — 지금 서버 렌더가 0.2~0.3초다.
