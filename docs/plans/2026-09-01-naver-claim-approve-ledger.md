...
- deploy: `6ac9f196`(T9) · `833aaf40`(AI_STATUS) · `c956fc60`(자산 핀) — CI 4/4 green.
- production: **PR #249 머지 `b0c81413`**. PR #247 은 그 사이 운영이 #245·#246 으로
  앞서가 충돌이 나 닫고 새 브랜치로 재제출했다(강제 푸시는 가드가 막는다).
- 승격 트리에서 **전체 스위트 직접 실행 7713 passed** — 승격 PR 은 본 스위트를 안 돈다.
- **자산 핀 사고를 승격 시험이 잡았다**: 운영이 `?v=20260901c` 로 **다른 자산**을
  서빙 중이었다(백필 자산). 그대로 갔으면 SW staticCacheFirst 라 워크벤치를 이미 연
  사용자가 옛 JS 를 계속 써 버튼이 아무 일도 안 했다. `20260902a` 로 올려 해소.
  하루 전 `957fc5ec` 가 고친 것과 **정확히 같은 함정**이다.
- **게이트 ON (사용자 지시)**: `web` 에 `FOMS_NAVER_CANCEL_APPROVE_ENABLED=1` ·
  `FOMS_NAVER_RETURN_APPROVE_ENABLED=1`, 재배포 `021de23c` (2026-09-01 23:23:12Z,
  변수 등록보다 뒤). healthz commit `b0c81413`. 새 JS 서빙 확인(200 + 승인 배선 4곳).
  worker 에는 넣지 않았다.
- 사용자가 단계 점등(취소 먼저 → 성공 확인 → 반품) 대신 **둘 다 즉시 ON** 을 골랐다.
  첫 실호출이 두 배선의 동시 검증이 되므로, 실패하면 어느 쪽 배선인지 갈리지 않는다는
  점을 기록해 둔다.
