...
| T2 쓰기 API + 레지스트리 4종 | PENDING | `pytest tests/domains -q` |
...
| T4 AS 탭 화면 | PENDING | 렌더 테스트 + 로컬 dev 확인 |
| T5 실측 대시보드 | PENDING | 렌더 테스트 + TTFB |
| T6 모바일 v3 | PENDING | 렌더 테스트 |
| T7 전체 게이트 | PENDING | `pytest tests/domains -q`, `APP_OK`, pre_push_smoke exit 0 |
