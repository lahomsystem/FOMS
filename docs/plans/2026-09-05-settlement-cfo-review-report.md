...
| 보류·한도 | 창 안 보류(음수)+해제(양수) **순증감**(타일 부제가 보류·해제 부호별 합을 따로 말한다, 2026-09-06 수정) | 적재 구간 전체 누적 잔액은 보류 상세 패널 머리 한 줄(보류 합·해제 합·구간). 짝 판정은 화면이 하지 않는다 |
...
| 전기 대비 % | 월 단위로 꽉 찬 달력 월(1일~말일)을 조회하면 직전 달력 월(2026-09-05 수정), 그 밖은 같은 **일수** 직전 구간 — 라벨에 구간(MM-DD~MM-DD)이 찍힌다. 기본 창은 미실현 14일 포함 | 월 보고는 1일~말일로 조회해 쓸 것 |
| 예외 N건(스트립·배지) | 상한 전 **모집단**(`exception_totals.total`, 2026-09-05 수정) — 표 머리가 "N건 중 M건 표시(갈래별 상한 50)"를 말한다. 종류 8개: UNMATCHED·UNLINKED·HOLDBACK·LIMIT·NEGATIVE·RETRO·COUNT_MISMATCH·**SYNC_FAILED**(2026-09-06 추가, 최신 실행이 실패면 1행) | 갈래별 건수는 `exception_totals` |
