...
| **R-2** | 유령 주문 띠 + `주문 취소 처리`(soft delete) | **완료** `7ac7abb8` · 스테이징 3건 실화면 확인 |
| 곁 | nav '네이버 수집' 채널 표식(인라인 SVG) | **완료** `6d14abf8` |
...
## 실측값 (2026-08-25 종료 시점)

- **운영 유령 주문 = 0건**(읽기전용 확인). 스테이징만 3건이다 — 운영은 개방 첫날이라
  아직 취소 이력이 쌓이지 않았다. **띠가 운영에서 비어 있는 것이 정상**이다.
- 스테이징 워크벤치 실데이터: 91집 · 상품주문 295건 · 손대지 않음 16집.

## 남은 것 (R-3 밖)

- **R-1·R-2·nav 표식 운영 승격** — 사용자 결정: **R-3 까지 만들고 한 꾸러미로 올린다.**
  오늘 승격한 것은 `099568a2`(sticky·도크·쿠폰·계정 생성)까지다.
