...
| 2 | 발송 칸이 `done` 이고 `dispatch_ours_at` 있음 | `{dispatch_ours_at} · ERP발송` | `.wb-st__when` |
| 3 | 발송 칸이 `done` 이고 `dispatch_ours_at` 없고 `dispatch_naver_at` 있음 | `{dispatch_naver_at} · 판매자센터` | `.wb-st__when` |
...
| `.wb-pipe__s--place` / `--ship` | 어느 축의 칸인가 | 템플릿이 칸마다 붙인다 — 색을 축별로 가르는 근거 |
| `.wb-pipe__s--place.wb-pipe__s--done` | 발주확인이 끝남 | **파랑**(`--wb-accent`). 발송 완료(초록)와 가른다 |
...
## 16. §15 이후 개정 (2026-08-31) — 실화면을 보고 사용자가 고친 것

운영 실화면을 사용자가 직접 보고 지적한 2건. **§15 와 충돌하면 이 절이 우선한다.**

### 16.1 끝난 칸 두 개가 같은 색이었다

`발주확인 완료` 와 `발송처리 완료` 가 둘 다 `.wb-pipe__s--done`(초록)이라, 파이프 두 칸이
나란히 초록이면 어느 칸이 어느 칸인지 안 보였다. **중간 단계(발주확인)는 파랑
(`--wb-accent`), 마지막 단계(발송)는 초록**으로 가른다 — 색이 곧 진행 방향이다.

축 표식(`--place`/`--ship`)은 **템플릿이 칸마다 붙인다**. `:first-child` 같은 순서 선택자로
칠하면 나중에 칸이 늘거나 순서가 바뀔 때 엉뚱한 칸이 파랗게 되고, 마크업에서 시험할 근거도
없다. 계약 테스트가 두 표식의 동시 존재를 잠근다
(`test_two_finished_steps_are_not_the_same_colour`).

### 16.2 발송 출처는 낱말 하나로

옛 꼬리는 `네이버 확인됨` / `판매자센터에서 직접` 두 문구였고 시각은 **둘 다 네이버 sendDate**
였다. 사용자 결정(2026-08-31): **시각 + 출처 한 낱말**로 단순화한다.

| 어떤 발송인가 | 꼬리 |
|---|---|
| 우리가 보냈다(`dispatch_ours_at` 있음) | `{ours_at} · ERP발송` |
| 판매자센터에서 나갔다(우리 기록 없음) | `{naver_at} · 판매자센터` |

**시각도 말하는 주체의 것을 쓴다.** 우리가 보낸 건에 네이버 sendDate 를 찍으면 `ERP발송` 이라
적어 놓고 네이버가 적은 시각을 보이게 되어 주체와 시각이 갈린다.

### 16.3 손대지 않기로 한 것 — `네이버 기록 없음`

같은 자리에서 "우리 발송 기록만 있고 네이버 스냅샷에 `sendDate` 가 없는" 집의 빨강 배지·경고
줄을 지우자는 안이 나왔다(방금 발송한 집은 다음 수집 전까지 항상 이 모양이라 대부분 사고가
아니라 낡은 스냅샷이다). **사용자 결정: 현재 그대로 둔다.** 판정(`_dispatch_view` 의
`mismatch`)·배지·경고 줄 셋 다 변경 없음. 다시 논의하려면 이 절을 먼저 읽어라 —
지우는 쪽으로 가면 "우리는 보냈는데 네이버에 실제로 안 들어간" 유실이 화면에서 사라진다.
