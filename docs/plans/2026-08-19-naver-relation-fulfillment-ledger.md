...
| T16-H | 버튼 노출 규칙(발주확인 먼저 · 추가결제만 여기서 발송처리) | T16-G | **DONE(코드)** `110c61d7`+`0f73fb14` | 관계별 노출 계약 6 green · 스테이징 렌더 확인(이대근 #4473 pane 에 '발주확인 보내기') · **버튼 클릭(실호출)은 사용자 몫** |
...
- 2026-08-20: 스테이징 렌더 확인 중 **막힌 경로 1개 발견·수정** — pane 이 확인 대기 큐 안에서만
  링크를 찾아, 주문이 붙은 건(전부 확인 완료 상태)은 `link_id` 를 줘도 큐 첫 건이 열렸다.
  발주확인·발송처리 버튼이 그 화면에 있어 처리 경로가 통째로 막힌다. 큐에 없으면 DB 에서
  직접 로드하도록 고쳤다.
- 2026-08-20: **CI 주의** — deploy FOMS CI red 는 타 세션 커밋 `3d3c2f61`(RESTORE-GUI-01 T1)의
  `ORDER_FIELD_RESTORED` 라벨 누락 + `events.api_restore_field_change` 정책 미분류 때문이다.
  이 원장 범위의 커밋은 아니다(내 몫이던 라벨 누락은 `7eb23912` 에서 이미 고쳤다).
