...
- [x] **문자 실발송 검증 완료** (2026-08-13): 1차 시도는 Solapi API 키 IP 허용 목록이 스테이징 IP(208.77.246.107) 403 차단 — 이때 `brand → brand_fallback` 재시도 로그로 T8.1 ③ 배선 실전 작동 실증. 사용자가 IP 제한 해제(모든 IP 허용) 후 재시도: **2통 발송 성공** — 주문 4404(발주사 無 → HAUD `sent:true, sender_source:brand`)·주문 4405(발주사 라홈 → LAHOM 분기 `sent:true`), 수신 010-8327-7282. 잔여물 정리 완료(share 20~22 회수·주문 4403~4405 soft delete). **잔여**: 수신 문자 2통 발신번호 육안 확인(1566-0703·1566-0792 순, 사용자) + **Solapi IP 허용 목록 원복**(현재 모든 IP 허용 = 보안 완화 상태 — 운영 static IP 등록으로 재잠금 권장, 단 Railway 운영도 static 아니면 해제 유지 판단은 사용자).
