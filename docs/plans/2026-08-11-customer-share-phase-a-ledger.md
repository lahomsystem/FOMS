...
| T8 | 문자 발송 (sender_phone·_solapi_send_text·멱등·버튼 배선) | `pytest tests/domains/test_order_share_sms.py tests/domains/test_write_guard.py tests/domains/test_auth_enforcement.py -q` PASS + audit 인벤토리 재생성 + APP_OK | DONE | PENDING_SHA | 64 passed(게이트 포함)·coverage 100%. 선점 insert 멱등=벤더 호출 1회 assert, 감사=수신번호 마스킹·토큰 미격납, 발신 폴백 2분기, senderphone_00 마이그레이션 |
