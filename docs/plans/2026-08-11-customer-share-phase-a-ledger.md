...
- [ ] **Solapi API 키 IP 허용 목록에 Railway IP 미등록 — 실발송 403 차단** (2026-08-13 발견): 스테이징 실발송 시도(주문 4403·share 20)가 `허용되지 않은 IP(208.77.246.107)` Forbidden. 콘솔 설정 PC IP만 허용된 상태. 조치 필요: 운영 static IP 3개(네이버 수집용으로 확보한 그 IP) 등록 (+스테이징 검증은 IP 제한 임시 해제 필요 — 스테이징은 static IP 없어 변동). **부수 실증**: 로그에 `source=brand → brand_fallback` 재시도 확인 — T8.1 ③ 재시도 배선 실전 작동. 앵커·감사(SHARE_SMS_SENT 실패 기록)도 정상. 테스트 잔여물 정리 완료(share 20 회수·주문 4403 soft delete).
