...
- [ ] **T13(설계 확정·미구현): 미저장 입력 반영 알림톡 발송** (사용자 요구 2026-08-19): 실측 알림톡 본문이 저장본 SSOT 라 저장 전 입력이 반영 안 됨. 해결안 = 알림톡 메뉴 클릭 시 dirty 감지 → 기존 통합 저장 호출(자동 저장) → 저장 성공 후 발송 모달(화면값 직접 조립은 저장본과 불일치 사고 위험이라 배제, draft 부활 레이스 함정 주의). 구현은 새 세션에서. **주의: 템플릿 WL 버튼이 운영 도메인 고정** — 스테이징 발송 시 버튼 링크는 운영 /s/로 가서 404(스테이징 검증 한계, 발송 성공 여부만 확인).
