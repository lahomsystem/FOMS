...
### 눈 확인은 **지금 성립하지 않는다** — 계정 잠금을 풀지 않은 이유

버튼이 실제로 뜨는지는 눈으로 못 봤다. 못 본 게 아니라 **볼 것이 없다.**

화면 조건이 게이트 단독이 아니다 — `naver_workbench_pane.html` 이
`can_return_reject = return_reject_enabled and return_rejectable_count > 0` 을 세우고,
**버튼과 모달을 둘 다 그 값으로 감싼다**(한쪽만 열면 열린 버튼이 빈 모달을 연다 — v2 결함).

그리고 운영에 거부 대상이 **0건**이다(2026-09-01 읽기 전용 조회, `read_only=True`·쓰기 없음):
...
| `RETURN_DONE` | 25 |
| `CANCEL_DONE` | 18 |
| **`RETURN_REQUEST`·`RETURN_REQUESTED`·`COLLECTING`** | **0** |

처음엔 `productOrder.claimStatus` 만 세었는데, `extract_claim` 은 그 값이 비면
`cancel`→`returnInfo`/`return`/`exchange`→`currentClaim` 으로 **폴백한다.** 폴백 경로까지
다시 세고도 0건이다 — 거짓 음성이 아니다.

**그래서 이 상태에서는 게이트가 켜져 있든 꺼져 있든 화면이 똑같다.** 잠금을 풀고 들어가도
"버튼 없음"만 보이고 그것은 게이트 때문인지 대상이 없어서인지 갈라 주지 않는다. 운영 측정
계정은 요청 1건당 1회라, **아무것도 증명 못 하는 데 그 1회를 쓰지 않았다**(사용자 결정
2026-09-01: 진짜 반품이 오면 그때 본다).

**다음 사람에게**: 이 확인을 하겠다고 계정 잠금부터 풀지 마라. 먼저 거부 대상이 1건이라도
있는지 세라(위 조회). 0건이면 화면은 볼 것이 없다. 스테이징 재현도 바로는 안 된다 —
`FOMS_NAVER_RETURN_REJECT_ENABLED` 미설정 · `FOMS_NAVER_WORKBENCH_COHORT=38`(실계정 차용은
정책 금지라 QA 계정이 들어가려면 코호트를 넓혀야 한다) · 반품 요청 링크 시드까지 세 가지가
필요하다.

배포 인과(변수 등록 뒤 부팅한 재배포 `09aeca29`)와 healthz 가 이 세션이 가진 근거의 전부다.
