...
| R-7 | **닫힘 + 운영 반영** — PR #190 `0db28c80` | 아래 |
| R-8 | **닫힘 + 운영 반영** — PR #190 `0db28c80` | 아래 |
...
**앞 세션 반품 점검이 찾은 결함 8건이 전부 닫히고 운영까지 반영됐다.**

| PR | 담은 것 | 운영 커밋 |
|---|---|---|
| #183 | R-5·R-6·R-3·R-2·R-1 | `07a0e119` |
| #186 | R-4 | `2ad638c4` |
| #190 | R-7·R-8 | `0db28c80` |

테스트 52개 추가(**음성 대조군 19개**). 실화면 확인은 R-5 만 가능했고 운영 8/8 깨끗.
나머지는 트리거 표본이 두 환경 모두 0건이라 테스트가 유일한 관문이다.

### 이 8건의 공통 뿌리

판정 축이 `label` 유무 · `blocking` 한 비트뿐이었다. 이번에 네 축을 `mapping` 에 세웠다:

| 축 | 묻는 것 |
|---|---|
| `claim_kind` | 취소인가 반품인가 교환인가 (`claimType` 우선, 없으면 상태 이름) |
| `MONEY_BACK_CLAIM_KINDS` | 돈이 되돌아가는 종류인가 (교환은 아니다) |
| `blocks_irreversible` | 네이버로 불가역 호출을 보내도 되는가 (진행 중이면 교환도 막는다) |
| `is_money_back_claim` | 결제액에서 환불이 아직 안 빠졌는가 (거부는 아니다) |
| `BLOCKING_CLAIM_STATUSES`(기존) | 주문을 만들면 안 되는가 |

**축을 하나로 뭉치려는 유혹이 이 결함들의 원인이었다.** 다섯 질문은 답이 서로 다르다 —
교환은 ②③에서 갈리고, 거부는 ③④에서 갈리며, 완료는 ③④에서 갈린다.

### 승격 규율 (이번에 세 번 데었다)

운영 승격은 **한 시간이면 낡는다**. #183→#182, #186→#185, #190→#187·#188 이 전부 승격
작업 **도중에** 들어왔고 두 번은 내가 건드리는 파일과 겹쳤다. 승격 트리에서 PR 만들기 직전
...
### ④ 낡은 '승격 대기' 줄 훑기 (2026-08-31)

오늘 두 번(①·도크 UX) 같은 모양을 만나서 **살아 있는 표면 전체**를 같은 방법으로 훑었다.
방법은 하나다 — 그 항목이 건드린 파일 목록을 커밋에서 뽑아
`git diff --numstat origin/production origin/deploy -- <그 파일들>` 이 비면 이미 운영에 있다.
**커밋 SHA 로는 판정할 수 없다**(승격이 cherry-pick 이라 SHA 가 안 맞는다).

| 줄 | 주장 | 실제 | 근거 |
|---|---|---|---|
| AI_STATUS 마법사 핫픽스 | 공유 폼 커서분 `c335cd83` 운영 승격 대기 | **이미 운영에** | 3파일 0줄 차이 · `restoreAmountCaret` 존재 · 핀 `20260829a` 운영 동일 |
| `2026-08-28-structured-source-dual-meaning-followup.md` | 뱃지 문구 `3aba2645` deploy 에만 | **이미 운영에** | 3파일 0줄 차이 |
| AI_STATUS 이력 상태 칸 | 잔여: 운영 승격·실화면 대조 | **승격은 끝났다** | PR #198 은 닫히고 #200 이 머지(production `639feabe`) · 관련 5파일 0줄 차이. 실화면 대조만 남음 |

**반대 방향도 하나 나왔다** — 어느 줄도 말하지 않는데 운영에 없는 것: `b084c655`
(네이버 관리 조작 감사 19곳에 행위자 `user_id` 주입, 2026-08-31 타 세션). `naver_ingest.py` 가
운영 대비 `+19` 로 남아 있다. 오늘 만든 것이라 낡은 게 아니라 정상 대기다 — 다만 **AI_STATUS 에
줄이 없으니 다음 세션이 못 본다**. AI_CHANGELOG 에는 `(deploy 대기)` 로 적혀 있는데 실제로는 이미
deploy 에 있다.

훑고도 안 고친 것: 옛 원장 본문의 "운영에 없다" 서술(네이버 전량 승격 이전 기록 등)은 **그 시점의
사실**이라 그대로 둔다. 고칠 대상은 지금 읽히는 표면(AI_STATUS 진행 중 · 살아 있는 원장의 잔여 줄)뿐이다.
...
| PR | 생성 | 판정 근거 |
|---|---|---|
| #133 (마이그레이션 체인 재배열) | 08-22 | 코드 파일 누락 0(운영에 없는 것은 문서 6개·`.mcp.json` 뿐) · 담긴 마이그레이션 9종 전부 운영에 있음 |
| #144 (알림톡 흔적 칩 T15) | 08-24 | 파일 28개 전부 운영 존재 · `describe_change`·`path_label`·`erp-alimtalk-trace.css` 운영 확인 |
| #49 (출고 시공일 변경 알림) | 08-05 | 파일 32개 전부 운영 존재 · `shipment_change_alerts.py`·`notifications/shipment_change.py`·`change_ack.py` 운영 확인 |

**판정은 커밋 목록이 아니라 내용으로 했다.** 셋 다 승격이 cherry-pick 이라 SHA 는 안 맞는다.
남은 diff 는 방향이 반대였다 — 예: `audit_message_display.py` 는 PR 쪽 +18줄 대 **운영 쪽
-222줄**. 즉 지금 머지하면 낡은 base 에서 3-way 병합이 일어나 **앞선 운영 코드를 되돌릴
위험**이 있었다(#113·#121 무효 전례와 같은 축).

부수로 운영 alembic 체인 건강을 확인했다 — **85 리비전, head 는 `merge_drawq_naverfail`
하나**. #133 이 하려던 재직렬화는 전제가 낡아 의미를 잃었다.

### 도구 함정 (기록)

head 세는 스크립트를 처음 짰을 때 **74개**가 나왔다. `down_revision: Union[str, None] = "x"`
에서 정규식이 타입 주석의 `None` 을 먼저 물어 부모를 통째로 못 읽은 것이다. 두 번째 판은
merge 리비전의 **여러 줄 튜플**을 못 읽어 5개가 나왔다. 정답은 1개다 — 체인 건강을 자동으로
판정할 때는 (1) 타입 주석 뒤의 `=` 부터 읽고 (2) 다음 최상위 대입 전까지를 통째로 읽어야 한다.
**"head 가 여럿"이라는 결과는 먼저 파서를 의심하라** — 부팅되는 운영에 head 가 74개일 수 없다.

열린 승격 PR 은 이제 `#1`(주간 리서치 봇) 하나뿐이다.
