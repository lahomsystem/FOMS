...
| T15-N | 스테이징 구석 QA — 관리 목록 카운트 단위·취소건 버튼 잠금 | T14-H | DONE | `3c4e8243`(deploy) · 눈 확인 완료 |
...
- ~~**운영 승격 시** IP 교체 필요~~ → **철회(2026-08-18 실측, §T15-E)**: 운영 `WORKER` 의 static egress IP 가 dev `worker` 와 **동일**(208.77.246.240~242)이라 교체 불필요·동시 운용 가능.
...
1. ~~IP 슬롯 3개를 dev worker 가 쓰고 있어 운영 승격 시 교체가 필요하다~~ —
   **2026-08-18 실측으로 철회(§T15-E)**: 두 프로젝트 worker 의 static egress IP 가
   `208.77.246.240/.241/.242`(sin/eqsg3a)로 **완전히 같다**. 교체 불필요,
   dev·운영 동시 운용 가능. region 이 바뀔 때만 재등록.
...
### T15-F CS 흐름 끝까지 시연 점검 (2026-08-18, 스테이징 실브라우저)

사용자 요청으로 **확인 큐 → 주문 만들기 → 규격 입력 → 확인 완료** 전 구간을 실제 클릭으로 밟았다
(gstack browse 헤드리스, `claude_master`). 대상: 김동은 집(link 121·122, 주문번호
`2026081820839951`, 본품 10개 + 1cm 옵션 24개) — 클레임 없는 깨끗한 건으로 골랐다.

| 단계 | 확인한 것 | 결과 |
|---|---|---|
| 큐 | 30집 / 상품주문 103건, 한 집 한 줄 + 구성 펼침 | PASS |
| 대조 pane | 2단계 안내(① 주문 만들기 ② ERP 규격 입력) · "담당자 지정은 실측 일정 잡을 때" · 원본 표(수취인·연락처·주소·금액·주문자·발송기한·결제·할인/쿠폰·상품ID/유입·**배송메모**) | PASS |
| 주문 만들기 | "2건 묶음" 버튼 → 주문 **#4468** 1건 생성, FOMS 현재 값 열이 즉시 채워짐, 콘솔 에러 0 | PASS |
| 편집기 도크 | `width_hints`: **총폭 3,240mm = 300mm × 10 + 10mm × 24** · 사양 불일치 경고 **"몰딩: 본품 무몰딩 · 추가 몰딩"** 이 실데이터에서 실제로 걸림 | PASS |
| 규격 입력 | W 3240 / D 600 / H 2400 저장 → `structured_data['items'][0]['spec_rows']` 에 기록 | PASS |
| 큐 복귀 | 배지 **✓ 주문 만들기 → ✓ ERP 규격 입력**, "규격 입력됨 — 확인 완료를 누르면 목록에서 빠집니다" | PASS |
| 확인 완료 | 2건 처리 → 목록에서 사라지고 네이버 탭 배지 103 → 101 | PASS |

**관찰 3가지**

1. **새 탭이 안 열렸다** — 헤드리스 팝업 차단 때문이고 코드 문제가 아니다. 차단돼도 pane 에
   "주문 열어서 규격 채우기" 링크가 남아 흐름이 끊기지 않았다(T14-G 의 팝업 차단 대비 설계대로).
2. `/edit/4468` **첫 접근 1회만** 로그인으로 튕겼다. 재로그인 후 같은 URL 정상, 이후 재현 없음.
   원인 미상 — 재발하면 그때 추적한다(현재 근거로는 세션 타이밍).
3. **`확인 완료` 는 묶음 안 링크 전부에 `reviewed_at`·`reviewed_by_user_id` 를 쓴다.**
   큐에서 빼는 기준이 `triage_state` 가 아니라 이 컬럼이다 — 되돌리려면 이 둘을 NULL 로.

**뒷정리**: 주문 #4468 은 소프트 삭제(status=DELETED·deleted_at, 휴지통에 남음),
link 121·122 는 `order_id=NULL · sync_status=COLLECTED · triage_state=NULL · reviewed_at=NULL`
로 원복. 큐가 **30집 / 103건**으로 복귀한 것까지 화면에서 확인했다.

### T15-G 큰 집(상품주문 14건) 시연 + **규격 도우미 오탐 발견** (2026-08-18)

대상: 전태곤 집(`2026081822487841`, link 126~139, 상품주문 **14건**) — 본품이 **2종**
(180cm ×2, 30cm ×14) + 1cm 옵션 ×2 + 수납구성 6종 + EP마감 2종 + 서랍 + 제로조인트 2종.

**흐름 결과 (전 단계 PASS)**: 큐 32집/120건 → "주문 만들기 14건 묶음" → 주문 **#4469** 1건
(items 14) → 편집기에서 항목 1·2 규격 저장 → 배지 ✓✓ → "확인 완료 14건" → 큐 **31집/106건**
(14건 빠짐). 콘솔 에러 0. 뒷정리 후 32집/120건 복귀 확인.

**총폭 힌트는 본품별로 따로 계산된다 — 설계대로 동작**:

| 본품 | 힌트 | 수식 |
|---|---|---|
| 180cm ×2 (+1cm ×2) | **3,620mm** | `1,800mm × 2 + 10mm × 2` |
| 30cm ×14 | **4,200mm** | `300mm × 14` |

수납구성(TYPE A~F)·EP마감·서랍·제로조인트는 폭에 더하지 않았다(설계대로).
1cm 옵션은 **첫 본품에 귀속**됐다 — 본품이 2종인 집에서 어느 본품에 붙일지는 원본에
정보가 없다. CS 가 눈으로 판단할 문제이고 힌트는 후보일 뿐이므로 그대로 둔다.

#### 발견: 사양 불일치 경고 **오탐** (몰딩 축, 실데이터 100건 중 9건)

`dock._axis_value()` 는 **상품명 + 옵션원문을 합친 문자열**에서 축 값을 읽고, 몰딩 축은
`무몰딩` 을 먼저 검사한다(부분문자열 함정 방어). 그런데 라홈 상품명이 라인 이름으로
`무몰딩` 을 달고 있고 고객이 옵션에서 `몰딩` 을 고르는 조합이 실재한다:

```
상품명 : 라홈 무몰딩 붙박이장 로라 시리즈 30cm 푸쉬타입 친환경 E0     → 무몰딩(상품명)
옵션   : 제품: 로라 몰딩 여닫이 30cm / 컬러: 화이트 / 손잡이: 피닉스바 → 몰딩(고객 선택)
1cm    : 로라 몰딩 여닫이 (푸쉬) 1cm                                  → 몰딩
```

본품·추가가 **둘 다 몰딩**인데 경고 `몰딩: 본품 무몰딩 · 추가 몰딩` 이 뜬다.
오늘 시연한 2집 모두 여기 걸렸고, 스테이징 본품류 링크 **100건 중 9건**이 같은 조합이다.
이대로 두면 CS 가 경고를 습관적으로 무시하게 된다(진짜 불일치를 놓치는 경로).

**근본 원인**: 축 값의 정본은 **고객이 고른 옵션 원문**인데 상품명과 같은 무게로 섞어 읽는다.

**수정 완료 (T15-H, 2026-08-18)**: `_row_axes()` 신설 — 축별로 **옵션 원문 우선, 그 축이
옵션에 없을 때만 상품명 fallback**. `build_width_hint` 의 본품·추가 판정이 둘 다 이걸 쓴다.

- 스테이징 원본 **128행 중 10행**의 축 판정이 바뀐다(전부 상품명 `무몰딩` vs 옵션 `몰딩` 조합).
- 진짜 불일치는 그대로 검출된다 — 옵션 원문에 축 값이 있기 때문(`제품: 로라 무몰딩 여닫이`
  본품 + `보테가 슬라이딩 1cm` = `문 방식` 경고 유지).
- 회귀 테스트 2개 추가: 옵션이 상품 라인 이름을 이긴다 / 진짜 불일치는 옵션에서 검출된다.
  `test_naver_dock` **29 passed** · integrations **208 passed** · smoke exit 0 · APP_OK.

**아직 남은 별개 사안(미수정)**: 한 집에 본품이 2종이고 1cm 옵션도 2종이면, 사람이 귀속을
지정하기 전까지 추정 귀속으로 비교하므로 짝이 어긋난 경고가 날 수 있다. 원본에 귀속 정보가
없어 CS 판단 몫이다(총폭 힌트도 같은 전제).

#### 부수 관찰: 스테이징 세션 만료

`claude_master` 세션이 시연 도중 **두 번** 끊겼다(그때마다 `/login` 리다이렉트, 재로그인 후 정상).
브라우저 자동화로 길게 붙어 있을 때만 보이는 현상이고 기능 결함 증거는 없다.
장시간 QA 스크립트는 **401/리다이렉트 시 재로그인하는 루프**를 넣어야 한다.

### T15-K 귀속 = 수집 순서 + 사양 보정 (2026-08-19 사용자 확정)

T15-I 의 순수 순서 규칙을 스테이징 36집 전수로 검증했더니 **본품 2개 이상인 집 4개 중 3개가
`M M a a a a`(본품 먼저 몰림)** 배치였다. 그 배치에서는 순서 규칙이 옵션을 전부 마지막
본품에 몰아 준다(예 `2026081435531421`: 몰딩 1cm 이 무몰딩 본품에 붙음).

| 배치 | 집 수 | 판정 |
|---|---|---|
| 본품 1개 | 32 | 옵션 전부 그 본품(자명) |
| `M a a … M a a`(섞임) | 1 | **수집 순서** |
| `M M a a …`(본품 먼저) | 3 | **사양 축 일치** — 아니면 미정 |

**구현**: 신규 모듈 `attribution.py` 에 판정을 모았다(매핑·도크가 같은 함수를 쓴다).
- 본품 사이에 옵션이 하나라도 끼면 = 순서가 정본 → 바로 위 본품.
- 본품이 앞에 몰렸으면 → 몰딩/문 방식/손잡이가 **정확히 한 본품과만** 맞을 때 그 본품,
  갈리거나 단서가 없으면 **미정**(도크에 "어느 본품의 구성인지 선택 필요").
- 축 값은 **옵션 원문 우선**(T15-H 규칙 그대로, 같은 모듈로 이관).
- 품목 금액은 잃지 않는다: 미정 옵션은 첫 본품에 붙여 합계를 보존하고, 화면에서는 계속
  "선택 필요"로 뜬다.

**실데이터 확인**(스테이징): `2026081435531421` → 몰딩 1cm 은 몰딩 본품, 무몰딩 1cm 은 무몰딩
본품 · `2026081667058371` → 몰딩 1cm 은 로라 몰딩 본품(보테가 아님) · `2026081822487841`(섞임)
→ 종전 순서 결과 유지.

**검증**: integrations **214 passed**(신규 2) · APP_OK.

### T15-L 승격 경로가 수집 순서를 깨뜨리던 버그 (2026-08-19 실사고)

T15-K 를 넣고 스테이징에서 전태곤 집(14건)을 **화면에서 다시 만들자** 추가옵션 12건이
180cm 본품 하나에 몰렸다(30cm 본품 0건). 로컬에서 같은 스냅샷을 id 순으로 넣으면 5/7 로
맞게 갈렸으므로 데이터가 아니라 **경로** 문제였다.

**원인**: `promotion._group_siblings()` 가 "기준 링크 우선, 나머지 id 순"으로 돌려줬다.
사람이 대표(금액 최대) 링크에서 버튼을 누르면 목록이 `M M a a a …` 로 재배열돼
귀속 판정이 **본품 먼저 배치**로 오인하고, 순서 규칙이 모든 옵션을 앞 본품에 몰아줬다.

**수정**: 기준 링크를 포함한 전체를 **id(수집) 순**으로 돌려준다. 대표 선정은 map_group 이
금액으로 따로 하므로 앞으로 끌어낼 이유가 없었다.
회귀 테스트: 대표 링크를 눌러 만들어도 항목이 본품 2행이고 금액 합계가 유지되는지 확인.

**교훈**: 귀속이 순서에 의존하는 순간 **그 순서를 만드는 모든 경로**(승격·수집·도크 쿼리)가
계약이 된다. 도크는 `order_by(id)` 라 무사했고 승격만 어긋나 있었다.

**검증**: integrations **215 passed**(신규 1) · pre_push_smoke exit 0 · APP_OK.

### T15-M 배포 후 눈 확인 — 전 항목 통과 (2026-08-19, 스테이징 `b76ef75c`)

- **품목 = 본품만**: 전태곤 집(상품주문 14건)을 화면에서 다시 만든 주문 **#4477** = 항목 **2행**,
  귀속 **7 / 5**(30cm 본품 7 · 180cm 본품 5), 금액 **2,946,660** = 묶음 합계 그대로.
  (T15-L 수정 전 같은 조작은 12/0 으로 몰렸다.)
- **도크 폭**: 1920px 셸에서 pane 실측 **720px**(종전 360px의 2배).
- **글자 크기**: 옵션 원문 **14px** · 도크 제목 15px · 귀속 셀렉트 14px
  (기준: erporder 제품 항목 입력 16px · 라벨 14px).
- **귀속 미정 0건**: `.naver-dock-assign.is-unset` 0 — 이 집의 옵션 12건이 전부 자동 귀속됐다.
- 콘솔 에러 0.

**주문 #4470 은 삭제**(입력값 0 확인 후) → **#4477 로 대체**. 그 사이 잘못 만들어진 #4474 도 삭제됨.

### T15-J 스테이징 배포 정체 — 눈 확인 보류 (2026-08-19)

`c199af5b`(T15-H)·`bd06b46a`(T15-I) 둘 다 **코드 게이트 green**(FOMS CI·PG Lane·Harness CI).
`perf-gate (staging)` 만 red 인데 **성능 회귀가 아니라** 게이트 첫 단계
`Wait for staging deploy (healthz commit == SHA)` 600초 타임아웃이다.

Railway FOMS-DEV `FOMS`(web) 상태(2026-08-19 GraphQL 조회):

- 빌드 2건 **FAILED** — 로그 마지막 줄이 `image push`(도커 11/11 성공 후 이미지 업로드).
  우리 Dockerfile·코드 문제가 아니다.
- 그 뒤 배포 **4건이 QUEUED** 로 쌓이고 BUILDING 으로 넘어가지 않는다.
- 스테이징은 계속 `b1a6cce5`(08-18 아침)을 서빙.

**사용자 결정: 손대지 않고 기다린다** — 큐 취소는 타 세션 배포를 끊는 행위다.
배포가 풀리면 확인할 것:

1. 편집기 도크 폭(1565px 이상 셸에서 720px).
2. 새 주문 **항목이 본품 수만큼**인지 + 출고가 합계 불변.
3. 도크 귀속이 **수집 순서**대로 붙는지(`⚠ 본품 선택…` 잔여 행 소멸).
4. 몰딩 오탐 경고 소멸 + 진짜 불일치(#4466 문 방식) 유지.

### T15-I 품목은 본품만 · 귀속은 수집 순서 · 도크 폭 2배 (2026-08-19 사용자 지시)

사용자 지시 3건. 원본 순서 규칙은 **사용자가 알려준 업무 사실**이라 추정보다 우선한다.

**1. 품목은 본품만 만든다** (`mapping.map_group`)
- 종전: 상품주문 1건 = 항목 1행 → 전태곤 집이 **항목 14행**(스크린샷). 규격 채울 본품 행을
  찾기 어렵다.
- 지금: 항목 = **본품 수만큼**. 추가옵션(수납구성·EP마감·서랍·제로조인트·길이추가)은 항목이
  아니라 그 본품 항목의 `naver_addons` 와 `options`(원문 이어붙임)로 들어간다.
- **금액은 잃지 않는다**: 본품 항목 금액 = 본품 + 귀속 옵션 합. 실데이터 검증(같은 집):
  상품주문 14 → 항목 2 · 30cm 본품 1,524,600 + 180cm 본품 1,422,060 = **2,946,660**
  = 묶음 합계, `items_total` 도 동일.
- `product` 문구의 `외 N건` 은 **상품주문 건수 기준 유지**(큐·수집 이력과 같은 셈법).

**2. 귀속은 수집 순서** (`mapping.split_main_groups`, `dock._guess_by_sequence`)
- 사용자 확인: 원본은 **본품 → 그 본품의 옵션들** 순서다(엑셀은 내림차순이라 옵션이 위로 뜬다).
- 실데이터 대조(`2026081822487841`): link 126 본품 180cm → 127~131 옵션 /
  link 132 본품 30cm → 133~139 옵션. 정확히 일치.
- 이름 토큰 유사도 추정(`_guess_main`)은 **삭제**했다 — 본품 이름이 비슷하면 엉뚱한 짝을
  만들었고(1cm 이 첫 본품으로 몰림) 총폭·불일치 경고가 함께 틀어졌다.
- 본품보다 먼저 온 옵션은 첫 본품에 붙인다. 사람이 고른 `assigned_main` 은 여전히 최우선.

**3. 도크 패널 폭 2배** (`static/css/orders/erp-naver-dock.css`)
- pane `360px` → `clamp(420px, 46%, 720px)`(상한 720 = 2배), 폼 열 `max-width` 도 같은 폭으로
  계산. 고정 720 은 1400px 셸에서 폼을 굶겨서 셸 폭 비례 + 상한 방식으로 했다.
- 좁은 셸용 서랍도 `min(390px, 86vw)` → `min(720px, 92vw)`.
- 자산 핀 `?v=20260818a` → `?v=20260819a` (SW staticCacheFirst 때문에 범프 필수).

**검증**: integrations **212 passed**(신규 5: 본품만 항목·순서 귀속·선행 옵션·도크 순서 귀속 2)
· 태블릿 셸 계약 포함 261 passed · APP_OK.

### T15-E 운영 IP 교체 — **불필요로 확정 (2026-08-18 실측)**

사용자 지적("dev·운영 IP 가 같을 것")을 Railway GraphQL 로 검증했다.

```
egressGateways(environmentId, serviceId)
FOMS-DEV / worker   → 208.77.246.240, .241, .242 (sin / eqsg3a)
FOMS-PRODUCTION / WORKER → 208.77.246.240, .241, .242 (sin / eqsg3a)   ← 완전 동일
FOMS-DEV / FOMS(web), FOMS-PRODUCTION / web → [] (static egress 미사용)
```

같은 region·zone 의 공용 egress 게이트웨이를 쓰기 때문에 **프로젝트가 달라도 IP 가 같다**.
따라서 지금까지 원장·가이드에 적혀 있던 **"운영 승격 시 IP 교체 필요 · dev 와 동시 운용 불가"는
사실이 아니다** — 커머스API센터에 이미 등록된 3개가 운영에도 그대로 유효하고, **스테이징과 운영
동시 운용도 가능**하다. 운영 켜기 단계는 ①계정 ②자격증명 ④수집 켜기 **3개로 줄었다**.

**남는 조건 1개**: 운영 WORKER 의 region 이 `sin` 에서 바뀌면 IP 도 바뀐다. 그때만 재등록한다.
...
② 운영 `WORKER` 에 자격증명 5종 ~~③ IP 교체~~(§T15-E — **불필요, IP 동일**)
③ `FOMS_NAVER_SYNC_ENABLED=1` 후 "지금 수집" 1회 확인.
...
### T15-N 스테이징 구석 QA 한 바퀴 (2026-08-19, claude_master)

지금까지 눈으로 안 본 자리를 훑었다. 데이터 변경 0건.

**고친 것 2개 (`3c4e8243` deploy, CI green, 스테이징 눈 확인 완료)**
1. **필터 버튼 카운트 단위 불일치** — 실화면이 `전체 36 · 수집됨(주문 전) 102 · 주문 생성됨 28`
   이었다. `전체`는 `_link_rows` 의 묶음(집) 총계인데 상태별은 링크 행 count 라 부분이 전체보다
   커 보였다. `_status_group_counts()` 로 `(묶음키, 상태)` 쌍을 한 번씩 세어 필터 술어와 같은
   단위로 맞추고, 묶음 키 SQL 식은 `_group_key_col()` 하나로 통일했다(총계·건수·페이지가 같은 식).
   라벨에 `집`을 붙여 단위를 화면에 명시. 배포 후: `전체 36집 · 수집됨 31집 · 주문 생성됨 6집`.
2. **취소·반품 건 '주문 만들기' 버튼이 관리 목록에서만 활성** — 확인 화면(`naver_triage.html`)은
   이미 `disabled` + 빨간 사유였는데 이력 화면은 배지만 달고 버튼이 살아 있었다. 서버
   (`promotion.py` blocking 가드)가 400 으로 막아 사고는 없었지만 헛클릭 후 실패 토스트만 떴다.
   행 dict 에 `claim_blocking`(묶음 내 any)을 실어 버튼을 잠그고 사유를 표시한다.
   배포 후 실측: 취소 완료 행 `disabled=true` + 사유 노출, 정상 29건은 그대로 활성(과차단 없음).

**정상 확인(수정 불요)**: 확인 화면 취소 차단 UI 실동작 · 원본 스냅샷 모달(JSON 4,035자) ·
콘솔 에러 0 · 실패 요청 0 · 확인 큐 헤더는 `N집 / 상품주문 M건`으로 단위 명시 ·
`_link_rows` 페이지 상한(PAGE_SIZE 50, 큐는 ×5=250)은 현재 대기 128건보다 커 잘림 없음.

**남긴 잔챙이(미수정)**: `?page=2` 같은 범위 밖 페이지는 빈 표 + 돌아갈 링크 없음
(total 36 < PAGE_SIZE 50 이라 페이저 자체가 안 그려진다). 실사용 경로 아님.

**환경 메모**: 스테이징 커머스API 만료일 `미등록` → T7 D-7 알림이 발동할 수 없다. 화면에
입력란은 없고 `NAVER_COMMERCE_APP_EXPIRES_ON`(가이드 §환경변수) 로만 넣는다. 운영 켤 때 챙길 것.

**보류였던 지도 좌표 이슈 — 원인 확정(네이버 문제 아님)**: 수집 주문 좌표가 안 잡히는 것은
`GEOCODE` outbox 의 **소비자(handler)가 아직 배포되지 않았기 때문**이다. 전 저장소에서
`register_handler(...)` 는 `STORAGE_DELETE` 하나뿐(`tools/ops/run_domain_side_effect_outbox.py:192`)이고,
워커 docstring 이 "handler 배포 전에는 delivery 를 켜지 않는다"고 설계로 못박아 뒀다.
따라서 주소 변경·신규 주문 **전 경로 공통**이며 네이버 수집 범위 밖 과제다.
