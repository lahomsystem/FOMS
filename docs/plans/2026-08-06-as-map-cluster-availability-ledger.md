...
| T6 | (사용자 승인 후) production 승격 — cherry-pick PR | PR 체크 green + merge + 운영 신코드 확인 | **DONE(2026-08-07)** — 사용자 승인(의존 `97364367` 출고알림 T1 포함) → **PR #54 merge, production `0f67dffd`**. 스트림 39커밋(미완료지도 2건은 운영 기승격 `b740bcb4`라 스킵, 재생성성 chore 3건은 승격 트리 재생성 `0b00db68`로 대체). 충돌=AI_STATUS 4·failopen 인벤토리 1(운영 계보 유지 기계 해소, 코드 충돌 0). 마이그레이션 0건. 검증: APP_OK·AS 도메인 30 passed·pre_push_smoke PASSED(253)·PR 체크 green(perf-gate 1m24s·pg-lane 1m35s, production 푸시 CI는 원래 없음=PR 체크가 정본)·운영 신코드 자산 확인(foms-as-round-chart.css 200 + as-dashboard.js 래퍼 실측 코드 서빙). promote 브랜치·worktree 정리 완료 |
...
| T15g | 데모 피드백(2026-08-07) — 지도 카드 확장 차트에서 판정/기록 **본문이 글자당 한 줄로 세로 표시**. 근본: 행 그리드 `92px 64px 1fr auto`가 날짜 92 + 유형 64 + 작성자 최대 104 + 간격·패딩 54를 본문(1fr)보다 먼저 배분 → 358px 컨테이너에서 본문에 **42px만 잔여**(실측). 패널 폭은 지도 소유라 확장 불가 → 행 레이아웃을 좁은 폭에서 2줄로 접음. 분기 기준은 뷰포트가 아니라 **부품 자신의 폭**(`container-type: inline-size` + `@container (max-width:560px)`) — PC 1920px에서도 지도 패널은 358px이라 미디어쿼리로는 틀린다. 동반: 회차 헤더 판정 플래그가 폭 독점해 요약이 "테…"로 뭉개지던 것(요약 basis 100%로 아랫줄 강제 — flex-wrap만으론 요약이 0까지 줄어 줄바꿈 미성립), 차트 헤더 버튼 라벨 세로 글자(헤더도 flex-wrap 허용) | 렌더 계약 테스트 + 3표면 실측 무회귀 + CI green | DONE — 원격 `f37ffdc9`(행) + `cb479dc5`(헤더), CI 4 워크플로 전수 green. **배포판 스테이징 실측**: ①지도 카드 358px — 판정 본문 42px→**320px**, 높이 171px→41.6px(2줄), 오버플로 0 ②모바일 카드 상세 294px — 3열 메타+본문 전폭, 헤더 항목 높이 62~92px→20.6~46px, 오버플로 0 ③PC 확장 720px — 그리드·높이(713.27)·오버플로 수정 전과 **완전 동일**(container-type 부작용 0) ④콘솔 에러 0(모바일·PC). ?v=20260807b. 진단 노하우: 주입 검증 시 대시보드는 CSS 링크가 body에 있어 head 주입 style이 **뒤로 밀린다**(주입은 body 끝에). **운영 승격 완료(사용자 승인 2026-08-07)**: PR #56 merge → production `29acc3f5`(코드 2 + 원장 1, 마이그레이션 0), 체크 green(perf-gate 1m17s·pg-lane 2m18s), 운영 CSS 신코드 서빙 확인(@container·head flex-wrap) |

## T15f 4차 보고 진단 (2026-08-07)
- 증상 보고: 확장 차트 입력부 우측 잘림, 크롬 줌 80~100%에서만·그 외 정상.
- 진단: 스테이징 헤드리스 실브라우저 실측 — 창 폭 7종(1280~2866)·devicePixelRatio 3종(1.0/1.125/1.25)·가로 스크롤 상태·**열린 채 리사이즈(동결 maxWidth 시나리오)** 전수에서 확장 박스 오버플로 0px, 미재현. 스테이징 서빙 자산 최신 확인(js `?v=20260806d`).
- 판정: 현행 코드 재현 불가 — **0b50c6f9(어제 10:05) 이전 빌드 탭** 추정. 사용자 강력 새로고침(Ctrl+F5) 후 줌별 재열기 확인 대기. 재현 확정 시 F12 콘솔 진단 스니펫으로 실환경 기하 캡처 예정.
- 알려진 설계 한계(잠복): 확장 열린 뒤 줌/창 크기 변경 시 인라인 maxWidth가 열 당시 값으로 동결(재열기로만 회복). 재현 확정 시 상시 추적(ResizeObserver)으로 근본 수정 후보.
