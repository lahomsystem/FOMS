...
| T6 | (사용자 승인 후) production 승격 — cherry-pick PR | PR 체크 green + merge + 운영 신코드 확인 | IN_PROGRESS(2026-08-07) — 승격 worktree `c:/tmp/foms-prom-as` 브랜치 `promote/as-map-t6` 준비 완료: 스트림 39커밋(의존 `97364367` 출고알림 T1 field_update SSOT 잠정 포함, 미완료지도 2건은 운영 기승격 `b740bcb4`라 스킵, 재생성성 chore 3건 제외). 충돌=AI_STATUS 4·failopen 인벤토리 1(전부 운영 계보 유지로 기계 해소, 코드 충돌 0). 인벤토리 3종 재생성 커밋 `0b00db68`. 마이그레이션 0건. APP_OK·AS 도메인 30 passed·pre_push_smoke PASSED(253). **PR 생성 전 사용자 확인 대기**: 의존 97364367 포함 승인 |
