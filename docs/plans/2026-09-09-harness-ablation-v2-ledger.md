...
| **합계(디스크 경로 5곳 — 세션 목록 전체가 아니다)** | **102** | **13,723** | **70** | **3,976.9 ≈ 3,977** | 위 5행 합 |

> **산출법 한계 2종(2026-09-10 `/context` 실측으로 드러남 — 이 표를 인용하기 전에 읽어라)**
> 1. **모집단 누락**: 위 5행은 디스크 경로(`~/.claude/skills` · `.claude/skills` · `.claude/commands` · 플러그인 캐시)만 훑는다. 내장 스킬(`dataviz` 480 · `claude-api` 360 · `design` 340 · `code-review` 270 등 17종 약 2,750토큰)은 어느 경로에도 파일로 없어 **구조상 0으로 잡힌다**. 그래서 합계를 '세션 목록 전체'로 읽으면 안 된다.
> 2. **항목 오버헤드 미계산**: 공식이 `description` 글자수만 센다. 스킬 이름과 목록 서식이 빠져 같은 caveman 20종에 공식 1,761 대 실측 약 2,200 으로 **약 25% 낮게** 나온다.
> 같은 잣대(내장 제외)로 맞춘 적용 전/후: 102종 3,977 → 37종 약 2,700~2,860(약 28% 감소). 다음 감사가 이 표를 다시 만든다면 `/context` 실측을 정본으로 삼고 이 공식은 보조로만 쓴다.
