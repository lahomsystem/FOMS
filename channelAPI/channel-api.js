...
class ChannelAPI {
  ...
  async getUserChats(params = {}) {
    try {
      const response = await this.client.get('/user-chats', { params });
      return response.data;
    } catch (error) {
      throw new Error(`사용자 채팅 목록 조회 실패: ${error.message}`);
    }
  }
   ...
   * @param {Object} params - 검색 파라미터 (since, until, limit 등)
   * @returns {Promise<Object>} API 응답
   */
  async getChatMessages(userChatId, params = {}) {
    try {
      const response = await this.client.get(`/user-chats/${userChatId}/messages`, { params });
      return response.data;
    } catch (error) {
      throw new Error(`채팅 메시지 조회 실패: ${error.message}`);
    }
  }
  ...
  async getMeasurementScheduleMessages(params = {}) {
    try {
      const MEASUREMENT_GROUP_ID = '229923'; // 실측스케쥴 그룹 ID
      const response = await this.client.get(`/groups/${MEASUREMENT_GROUP_ID}/messages`, { 
        params,
        timeout: 60000 // 60초 timeout 적용
      });
      return response.data;
    } catch (error) {
  ...
  async getSalesOrderMessages(params = {}) {
    try {
      const SALES_ORDER_GROUP_ID = '209990'; // 영업팀_발주정보 그룹 ID
      const response = await this.client.get(`/groups/${SALES_ORDER_GROUP_ID}/messages`, { 
        params,
        timeout: 60000 // 60초 timeout 적용
      });
      return response.data;
  ...
  async getOrderRoomMessages(params = {}) {
    try {
      const ORDER_ROOM_GROUP_ID = '229625'; // 발주방 그룹 ID
      const response = await this.client.get(`/groups/${ORDER_ROOM_GROUP_ID}/messages`, { 
        params,
        timeout: 60000 // 60초 timeout 적용
      });
      return response.data;
