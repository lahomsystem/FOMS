/** API가 HTML(401/404/500)을 돌려주면 r.json()이 SyntaxError를 던짐. JSON일 때만 파싱 */
        ...
        `;
            ...
            (() => {
              const pay = sd.payments || {};
              const totals = sd.totals || {};
              const itemsTotal = Number(totals.items_total) || items.reduce((s, it) => s + (Number(it.price) || 0), 0);
              let depositAmt = Number(pay.deposit) || 0;
              if (!depositAmt && pay.deposit && typeof pay.deposit === 'object') depositAmt = Number(pay.deposit.amount) || 0;
              const remainAmt = Math.max(0, itemsTotal - depositAmt);
