/** API가 HTML(401/404/500)을 돌려주면 r.json()이 SyntaxError를 던짐. JSON일 때만 파싱 */
                ...
                <div class="dw-product-info-form">
                  ...
                  <div class="d-flex flex-column align-items-end gap-2 mt-2 erp-amount-block">
                    ...
                    <div id="erp-remaining-section-${orderId}-${idx}" class="d-flex justify-content-between justify-content-md-end align-items-center gap-2 w-100" style="max-width: 16rem; display: none;">
                      ...
                      <div class="badge bg-primary text-end erp-amount-value" id="erp-remaining-amount-${orderId}-${idx}">0원</div>
                    </div>
                  </div>
                </div>
                ${attachPanelHtml}
        ...
        `;
            ...
            (() => {
              ...
              const storedFinal = totals.final_amount == null ? totals.balance_amount : totals.final_amount;
              const remainAmt = storedFinal == null ? Math.max(0, itemsTotal - depositAmt) : Math.max(0, coerceAmount(storedFinal));
        ...
        // =====================================================================
        // ERP Dashboard Reload Logic (for when returning from edit page)
        // =====================================================================
        (function () {
