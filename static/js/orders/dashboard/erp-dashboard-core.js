/** Sync globals used by detail DOM (MY_ID / 팀 등) from .erp-dashboard data-* — runs on full load and fragment rebind. */
        ...
        /** ERP Order 일정 카드: 날짜 + 선택 시간 (structured_data.schedule.*) */
        ...
        function showErpToast(message, type = 'info') {
          ...
          toast.className = 'erp-toast erp-toast-' + (type === 'success' || type === 'error' ? type : 'info');
        ...
        var CAN_EDIT_ERP = (document.querySelector('.erp-dashboard')?.dataset?.canEditErp || 'false') === 'true';
        ...
        var __orderDetailLoadGen = {};

        /**
         * 주문 상세의 첨부 캐시와 셸/첨부 상태 플래그를 모두 초기화한다.
         * 첨부 업로드/삭제가 성공한 직후 호출하면, 다음 상세 펼침 시 최신 목록을 다시 받아온다.
         */
        function invalidateOrderDetailAttachments(orderId) {
          if (!orderId) return;
          if (typeof window.invalidateOrderDetailRuntimeState === 'function') {
            window.invalidateOrderDetailRuntimeState(orderId, {
              cache: __attachmentsCache,
              cacheAt: __attachmentsCacheAt,
              loadGen: __orderDetailLoadGen,
              containerPrefix: 'order-detail-content-'
            });
            return;
          }
          delete __attachmentsCache[orderId];
          delete __attachmentsCacheAt[orderId];
          delete __orderDetailLoadGen[orderId];
          const c = document.getElementById('order-detail-content-' + orderId);
          if (c) {
            delete c.dataset.loaded;
            delete c.dataset.shellLoaded;
            delete c.dataset.attachPhase;
            delete c.dataset.attachError;
            delete c.dataset.itemCount;
          }
        }
        window.invalidateOrderDetailAttachments = invalidateOrderDetailAttachments;

        var __currentAttachmentList = [];
        var __currentAttachmentIndex = 0;
        var __drawingCurrentFilesByOrder = {};
