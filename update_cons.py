...
def main():
    ...
    # Match either order of variable declarations before async function doUploadOne
    # Sometimes it's let ok = 0; const totalFiles = files.length; const CONCURRENCY = 3;
    # Sometimes it's const CONCURRENCY = 3; let ok = 0; const totalFiles = files.length;
    pattern = re.compile(
        r"(?:let ok = 0;[\s\n\r]*const totalFiles = files\.length;[\s\n\r]*const CONCURRENCY = 3;|const CONCURRENCY = 3;[\s\n\r]*let ok = 0;[\s\n\r]*const totalFiles = files\.length;)[\s\n\r]*async function doUploadOne.*?if \(statusEl\) statusEl\.textContent = `업로드 중.*?`;[\s\n\r]*}",
        re.DOTALL
    )

    matches = pattern.findall(content)
    if not matches:
        print("No matches found in construction scripts!")
        return
        
    print(f"Found {len(matches)} matches in construction scripts.")
