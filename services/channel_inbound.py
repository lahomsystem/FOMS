...
from models import ChannelInboundEventLog, Order, OrderTask
from db import get_db, db_session
from services.jobs.queue import enqueue_channeltalk_inbound
import os
...
def receive_webhook(payload: dict) -> Tuple[int, Dict[str, Any]]:
    ...
    if existing_log:
        terminal_statuses = ['worker_processing', 'created', 'parse_failed', 'rejected_signature', 'rejected_replay', 'rejected_group', 'ignored_duplicate']
...
def parse_order_text(text: str) -> Tuple[bool, dict, list]:
    """
    텍스트 파싱 로직 (CT-E-02)
    ...
    if name_match: data['customer_name'] = name_match.group(1).strip()
    else: missing.append('고객명')
        
    if phone_match: data['phone'] = phone_match.group(1).strip()
    else: missing.append('연락처')
        
    if addr_match: data['address'] = addr_match.group(1).strip()
    else: missing.append('주소')
        
    if product_match: data['product'] = product_match.group(1).strip()
