...
2. **한 커밋 = 한 논리적 변경**
3. **커밋 메시지**: 한글로 알기 쉽게 정리 → 커밋 후 푸시. **Win11 한글 방지**: `git config core.quotepath false` 및 `i18n.commitEncoding`/`i18n.logOutputEncoding` utf-8 설정. 한글 메시지는 **반드시 UTF-8 파일에 저장 후** `git commit -F 파일경로` 사용 (`-m "한글"` 사용 금지, PowerShell 인코딩 깨짐). **커밋과 푸시(Push)가 완료된 직후에는 작업 공간 정리를 위해 동원된 임시 텍스트 파일(예: `msg.txt`)을 반드시 커맨드로 삭제(`del` 또는 `rm`)하십시오.**
