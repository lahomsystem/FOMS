...
def run():
    with open("templates/erp_as_dashboard.html", "r", encoding="utf-8") as f:
        content = f.read()

    old_target = """        asUploadInput.addEventListener('change', async function () {
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            try {
              ...
              if (!sessRes.ok || !sessData.success) throw new Error(sessData.message || '세션 생성 실패');
              var putRes = await fetch(sessData.upload_url, {
                method: 'PUT',
              ...
              var completeRes = await fetch('/api/orders/' + orderId + '/attachments/complete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: sessData.key, filename: file.name, category: category, size: file.size })
              });
              var completeData = await completeRes.json();
              if (!completeData.success) throw new Error(completeData.message || '등록 실패');
              ok++;
            } catch (err) {
              console.error('AS upload error:', err);
            }
            if (asUploadStatus) asUploadStatus.textContent = '업로드 중... (' + (i + 1) + '/' + files.length + ')';
          }
          if (asUploadStatus) {
...
"""
    if old_target in content:
        ...
        with open("templates/erp_as_dashboard.html", "w", encoding="utf-8") as f:
            f.write(content)
        print("Updated AS Dashboard")
    else:
        # Debugging: Try replacing \n with \r\n
        if old_target.replace('\n', '\r\n') in content:
            content = content.replace(old_target.replace('\n', '\r\n'), new_logic.replace('\n', '\r\n'))
            with open("templates/erp_as_dashboard.html", "w", encoding="utf-8") as f:
                f.write(content)
            print("Updated AS Dashboard with CRLF")
        else:
            print("Could not find target string in AS Dashboard")
