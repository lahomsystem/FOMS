...
스킵은 정상 경로라 로그를 남기지 않는다(CLAUDE_HOOK_LOG 의 84% 가 이 정보성 행이었다).
실패해도 fail-open 하되 사유를 CLAUDE_HOOK_LOG에 남긴다.

2026-09-10 ablation v2: CONCURRENT-EDIT-LIVE/COLLISION 경고(디바운스 상태 파일 포함)를
제거했다 — 세션들이 격리 워크트리로 옮겨간 뒤 9일간 발화 0(원장 H-28·H-29).
"""
import json
import os
import sys
...
def _process(payload: dict) -> None:
    ...
    # 트리밖 편집(전역 메모리·스크래치패드 등)은 EDIT_LOG·pending 모두 스킵(무기록).
    ...
    append_edit_row(
        harness_runtime_path("EDIT_LOG.md"), rel_path, tool_name, session=_session_tag(payload)
    )
