...
def _wam_error(message: str, status_code: int, error_code: str):
    ...
    return render_template("channel_wam_error.html", message=message), status_code
...
def _resolve_html_template() -> str:
    if not get_wam_feature_flags().get("wam_v2", True):
        return "channel_wam_index.html"
    return "channel_wam/index.html"
...
def redirect_short_wam_link(token):
    ...
    if not payload:
        return render_template("channel_wam_error.html", message="Invalid or expired link"), 401
