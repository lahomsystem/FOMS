...
try {
    ...
    # tests/harness/load -> repo root is three levels up
    $repoRoot = (Resolve-Path (Join-Path $scriptDir "..\..\..")).Path
    $testScript = Join-Path $scriptDir "foms_150_realistic.js"
    $cookieFile = Join-Path $scriptDir "session_cookies.txt"
    $resultsDir = Join-Path $scriptDir "results"
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $summaryJsonRel = "tests/harness/load/results/$($TestProfile)_$($timestamp).json"
    $summaryTxtRel = "tests/harness/load/results/$($TestProfile)_$($timestamp).txt"
