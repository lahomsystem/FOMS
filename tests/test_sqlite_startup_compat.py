...
import apps.api.attachments as attachments
import foms.services.db_indexes as db_indexes
from safe_schema_migration import SafeSchemaMigration
...
def test_attachment_bootstrap_adds_columns_on_sqlite(monkeypatch) -> None:
    ...
    session = sessionmaker(bind=engine)()
    monkeypatch.setattr(attachments, "get_db", lambda: session)
    monkeypatch.setattr(attachments, "ensure_order_attachment_user_fk_set_null", lambda db: None)

    assert attachments.ensure_order_attachments_category_column() is True
    assert attachments.ensure_order_attachments_item_index_column() is True
    assert attachments.ensure_order_attachments_user_id_column() is True
