...
def test_clipped_object_is_restored_up_to_the_last_whole_entry():
    """저장 상한에서 잘린 객체도 읽을 수 있게 되살리고, 잘린 사실을 ``…`` 로 밝힌다."""
    rendered = amd.format_value(
        None,
        '{"address_note": "경비실 호출", "construction_note": "오전만", "measurement_n…',
    )

    assert rendered.startswith("주소 특이사항 경비실 호출, 시공 특이사항 오전만")
    assert rendered.endswith("…")


def test_object_with_no_content_says_so_instead_of_showing_json():
    """빈 칸만 담긴 객체는 JSON 원문이 아니라 "(내용 없음)" 으로 읽힌다."""
    assert amd.format_value(None, '{"address_note": "", "construction_note": "", "measurement_n…') == (
        "(내용 없음)"
    )
    assert amd.format_value(None, "{}") == "(내용 없음)"
