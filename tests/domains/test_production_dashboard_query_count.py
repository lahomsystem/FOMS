"""P2: /erp/production/dashboard route별 쿼리 수 예산 계약 (N+1 회귀 로컬 차단).
...
시드 함정: 생산 대시보드 쿼리(build_production_orders_query)는 flat 컬럼 erp_stage_code를
IN(['고객컨펌','생산','시공','CONFIRM','PRODUCTION','CONSTRUCTION']) 로 스코프한다.
erp_stage_code는 sync(명시 호출)가 아니라 raw seed로 직접 세팅해야 스코프에 걸린다.
"""
...
# warmup 후 격리 실측: small(4건)=7, big(12건)=7 → delta=0 (완전 배치, per-row 없음).
# 여유 +2로 예산 2.
ALLOWED_DELTA = 2
# 절대 상한: 현 실측 big(7) + 30% ≈ 10.
ABS_QUERY_CAP = 10
...
def _seed_order(idx: int) -> int:
    ...
    order = Order(
        ...
        customer_name=f"생산{idx}",
        phone=f"010-2000-{idx:04d}",
        address=f"서울시 생산구 {idx}",
        product="붙박이장",
        ...
        structured_data={
            ...
            "parties": {
                "customer": {"name": f"생산{idx}", "phone": f"010-2000-{idx:04d}"},
                "manager": {"name": "생산담당"},
            },
            "site": {"address_full": f"서울시 생산구 {idx}"},
    ...
    for j in range(2):
        db_session.add(
            OrderAttachment(
                order_id=order.id,
                filename=f"p{idx}_{j}.jpg",
                file_type="image",
                category="measurement",
                file_size=10,
                storage_key=f"p/{idx}/{j}.jpg",
...
def test_production_dashboard_fragment_200(client):
    """fragment 헤더로 실제 200을 반환하는지(무의미 404 계약 방지)."""
    _login_admin(client)
    for i in range(4):
        _seed_order(i)
    resp = _fragment_get(client)
    assert resp.status_code == 200, f"/erp/production/dashboard fragment -> {resp.status_code}"


def test_production_dashboard_no_n_plus_one(client):
