...
def test_mobile_surfaces_parent_cachebuster_bumped() -> None:
    ...
    assert "foms-mobile-surfaces.css') }}?v=20260716a" in layout_head
...
def test_w16_layout_head_loads_bundle_for_v2_and_v3_cohort() -> None:
    ...
    assert "foms-tablet-bundle.css') }}?v=20260715a" in layout_head
...
def test_calc_skin_wired_in_calculator_template_with_cachebuster() -> None:
    ...
    m = re.search(r"tablet-skin\.css'\s*\)\s*}}\?v=20260716a", html)
    assert m is not None, "calculator.html 에 tablet-skin.css ?v=20260716a <link> 부재"
