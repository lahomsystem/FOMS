...
def test_strict_canonical_templates_channel_wam_absorbed_under_channel() -> None:
    """§2.2.1: WAM human-facing templates live under templates/channel/, not templates/channel_wam/."""
    assert (ROOT / "templates" / "channel" / "wam" / "index.html").is_file()
    assert (ROOT / "templates" / "channel" / "wam_index.html").is_file()
    assert (ROOT / "templates" / "channel" / "wam_error.html").is_file()
    assert not (ROOT / "templates" / "channel_wam").exists()
...
def test_channel_wam_index_wrapper_renders_v1_fallback_shell():
    ...
    with app.test_request_context("/channel/wam/?launch_token=test"):
        html = render_template(
            "channel/wam_index.html",
            summary=summary,
            attachments=attachments,
            token="test",
        )

    assert "css/contexts/channel/tokens.css" in html
